/* ============================================================
   HubISoccer -- agent-cv.js
   Page Mon CV Pro - Espace Agent FIFA
   ------------------------------------------------------------
   ...
   - supabaseAuthPrive_agent_scouting -> lecture seule
     (annees_experience deja saisie ailleurs, affichee ici sans
     ressaisie)
   - supabaseAuthPrive_agent_cv       -> table de CETTE page
     (SQL : agent-cv-table.sql, sans RLS)
   ------------------------------------------------------------
   Une SEULE fiche par agent (agent_id UNIQUE), creee
   automatiquement au premier chargement. parcours et palmares
   sont des tableaux JSONB internes a cette fiche.
   ============================================================ */
'use strict';
...
const CV_TABLE        = 'supabaseAuthPrive_agent_cv';
...
/* ---------- 9. ANNEES D'EXPERIENCE (deja saisies ailleurs) ---------- */
...
/* ---------- 10. CHARGEMENT (OU CREATION) DE LA FICHE CV ---------- */
async function loadOrCreateCvRecord() {
    ...
    if (error) {
        hideLoader();
        console.warn('Table ' + CV_TABLE + ' absente :', error.message);
        showToast('Table du CV absente. Executez le script SQL <b>agent-cv-table.sql</b> dans Supabase.', 'warning');
        cvRecord = null;
        return;
    }
    ...
    const { data: created, error: createError } = await supabaseClient
        .from(CV_TABLE)
        .insert([{ agent_id: agentProfile.hubisoccer_id, parcours: [], palmares: [] }])
        .select()
        .single();
    hideLoader();
...
/* ---------- 11. PRE-REMPLISSAGE DE LA PRESENTATION ---------- */
function populatePresentation() {
    if (!cvRecord) { return; }
    document.getElementById('cvSpecialites').value = cvRecord.specialites || '';
    document.getElementById('cvPresentation').value = cvRecord.presentation || '';
    document.getElementById('cvVisible').checked = cvRecord.visible_public !== false;
}

/* ---------- 12. STATS RAPIDES ---------- */
function updateStats() {
    const parcours = (cvRecord && Array.isArray(cvRecord.parcours)) ? cvRecord.parcours : [];
    const palmares = (cvRecord && Array.isArray(cvRecord.palmares)) ? cvRecord.palmares : [];

    setText('statParcours', parcours.length);
    setText('statTitres', palmares.length);
    setText('statExperience', (scoutingData && scoutingData.annees_experience) ? scoutingData.annees_experience : '\u2014');
    ...
    if (chip) {
        chip.innerHTML = visible
            ...
            : '<i class="fas fa-eye-slash" style="color:var(--gray);"></i> Prive';
    }
}

/* ---------- 13. ENREGISTRER LA PRESENTATION ---------- */
async function enregistrerPresentation() {
    if (!cvRecord) {
        showToast('Chargement en cours, reessayez dans un instant.', 'warning');
        return;
    }
    const payload = {
        specialites    : (document.getElementById('cvSpecialites')?.value || '').trim() || null,
        presentation    : (document.getElementById('cvPresentation')?.value || '').trim() || null,
    ...
    cvRecord = updated;
    showToast('Votre presentation a ete mise a jour.', 'success');
    updateStats();
}

/* ================================================================
   PARCOURS -- ETAPES CLES
   ================================================================ */
function parcoursTrie() {
    const liste = (cvRecord && Array.isArray(cvRecord.parcours)) ? cvRecord.parcours.slice() : [];
...
function renderParcours() {
    ...
    container.querySelectorAll('.btn-delete').forEach(function(btn) {
        btn.addEventListener('click', function() { supprimerParcours(btn.dataset.id); });
    });
}

function chargerHistDansFormulaire(id) {
    const h = (cvRecord.parcours || []).find(function(x) { return String(x.id) === String(id); });
    if (!h) { return; }
    editingHistId = h.id;
...
function resetFormulaireHist() {
    ...
    if (title) { title.innerHTML = '<i class="fas fa-road"></i> Ajouter une etape'; }
}
...
async function enregistrerParcours() {
    ...
    if (!annee) { showToast('L\'annee est obligatoire.', 'warning'); return; }
    ...
    const liste = (cvRecord.parcours || []).slice();
    if (editingHistId) {
    ...
    if (error) { showToast('Erreur : ' + error.message, 'error'); return; }
    cvRecord = updated;
    showToast(editingHistId ? 'Etape mise a jour.' : 'Etape ajoutee a votre parcours.', 'success');
    resetFormulaireHist();
    updateStats();
    renderParcours();
}
...
async function supprimerParcours(id) {
    const h = (cvRecord.parcours || []).find(function(x) { return String(x.id) === String(id); });
    const titre = h ? h.titre : 'cette etape';
    if (!confirm('Supprimer « ' + titre + ' » de votre parcours ?')) { return; }
    const liste = (cvRecord.parcours || []).filter(function(x) { return String(x.id) !== String(id); });
    ...
    showToast('Etape supprimee.', 'info');
    updateStats();
    renderParcours();
}

/* ================================================================
   PALMARES & DISTINCTIONS
   ================================================================ */
...
function chargerPalDansFormulaire(id) {
    const p = (cvRecord.palmares || []).find(function(x) { return String(x.id) === String(id); });
    if (!p) { return; }
    editingPalId = p.id;
...
async function enregistrerPalmares() {
    ...
    if (error) { showToast('Erreur : ' + error.message, 'error'); return; }
    cvRecord = updated;
    showToast(editingPalId ? 'Distinction mise a jour.' : 'Distinction ajoutee au palmares.', 'success');
    resetFormulairePal();
    updateStats();
    renderPalmares();
}

async function supprimerPalmares(id) {
    const p = (cvRecord.palmares || []).find(function(x) { return String(x.id) === String(id); });
    const nom = p ? p.titre : 'cette distinction';
    if (!confirm('Supprimer « ' + nom + ' » du palmares ?')) { return; }
    const liste = (cvRecord.palmares || []).filter(function(x) { return String(x.id) !== String(id); });
    ...
    if (error) { showToast('Erreur : ' + error.message, 'error'); return; }
    cvRecord = updated;
    if (String(editingPalId) === String(id)) { resetFormulairePal(); }
    showToast('Distinction supprimee.', 'info');
    updateStats();
    renderPalmares();
}
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    if (btnSaveMission) { btnSaveMission.addEventListener('click', enregistrerPresentation); }

    const btnSaveHist = document.getElementById('btnSaveHist');
    if (btnSaveHist) { btnSaveHist.addEventListener('click', enregistrerParcours); }
