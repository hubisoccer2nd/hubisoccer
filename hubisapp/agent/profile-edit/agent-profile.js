/* ============================================================
   ...
   Page Mon Profil - Espace Agent FIFA
...
async function saveProfile(e) {
    ...
    Object.assign(agentProfile, updates);
}
...
function updateDeletionUI() {
    ...
    if (agentProfile.deletion_requested && !agentProfile.deletion_revoked) {
        if (delBtn) { delBtn.style.display = 'none'; }
        if (revokeBtn) { revokeBtn.style.display = 'inline-flex'; }
        if (status && agentProfile.deletion_requested_at) {
            const reqDate = new Date(agentProfile.deletion_requested_at);
...
async function confirmerSuppression() {
    ...
    if (error) { showToast('Erreur : ' + error.message, 'error'); return; }
    agentProfile.deletion_requested = true;
    agentProfile.deletion_requested_at = new Date().toISOString();
    agentProfile.deletion_revoked = false;
...
async function revokeDeletion() {
    ...
    if (error) { showToast('Erreur : ' + error.message, 'error'); return; }
    agentProfile.deletion_requested = false;
    agentProfile.deletion_revoked = true;
    updateDeletionUI();
