/* ============================================================
   HubISoccer -- agent-settings.js
   Page Parametres - Espace Agent FIFA
...
async function saveSettings(e) {
    ...
    else { showToast('Paramètres enregistrés. ✅', 'success'); Object.assign(agentProfile, updates); }
}
...
function exportData() {
    const dataStr = JSON.stringify(agentProfile, null, 2);
    const blob = new Blob([dataStr], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'hubisoccer_' + (agentProfile.hubisoccer_id || 'export') + '.json';
    a.click();
