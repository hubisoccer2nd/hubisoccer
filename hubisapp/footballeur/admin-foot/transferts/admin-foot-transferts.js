...
function formatMoney(value) { if (!value || isNaN(value)) return '—'; return new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'XOF', maximumFractionDigits: 0 }).format(value); }
...
async function loadFootballeurs() {
    const { data, error } = await supabaseClient.from('supabaseAuthPrive_profiles').select('hubisoccer_id, full_name').eq('role_code', 'FOOT').order('full_name');
    if (error) { console.error('Erreur chargement footballeurs:', error); return []; }
...
async function loadTransfers() {
    ...
    try {
        ...
        transfersData = (data || []).map(t => ({ ...t, footballeur_name: t.footballeur?.full_name || 'Inconnu' }));
        renderTransfers();
        updateStats();
    } catch (err) { console.error(err); showToast('Erreur chargement transferts', 'error'); }
...
function renderTransfers() {
    ...
    const filtered = transfersData.filter(t => {
        const matchesSearch = (t.footballeur_name || '').toLowerCase().includes(search) || (t.club_depart || '').toLowerCase().includes(search) || (t.club_arrivee || '').toLowerCase().includes(search);
    ...
    if (!container) return;
    if (!filtered.length) { container.innerHTML = '<p style="text-align:center;color:var(--text-muted);padding:40px;">Aucun transfert trouvé.</p>'; return; }
    const statusLabels = { pending: 'En attente', approved: 'Validé', rejected: 'Rejeté' };
    container.innerHTML = filtered.map(t => {
        const typeLabel = { transfert: 'Transfert', pret: 'Prêt', fin_contrat: 'Fin de contrat' }[t.type_transfert] || 'Transfert';
...
async function loadOffers() {
    ...
    try {
        ...
        if (error) throw error;
        offersData = (data || []).map(o => ({ ...o, footballeur_name: o.footballeur?.full_name || 'Inconnu' }));
        renderOffers();
        updateStats();
    } catch (err) { console.error(err); showToast('Erreur chargement offres', 'error'); }
    finally { showLoader(false); }
}
...
function renderOffers() {
    ...
    const filtered = offersData.filter(o => {
        const matchesSearch = (o.footballeur_name || '').toLowerCase().includes(search) || (o.from_entity || '').toLowerCase().includes(search) || (o.title || '').toLowerCase().includes(search);
    ...
    if (!container) return;
    if (!filtered.length) { container.innerHTML = '<p style="text-align:center;color:var(--text-muted);padding:40px;">Aucune offre trouvée.</p>'; return; }
...
document.getElementById('transferForm').addEventListener('submit', async (e) => {
    ...
    if (!user_hubisoccer_id || !date_transfert) { showToast('Footballeur et date requis', 'warning'); return; }
    showLoader(true);
    try {
        const payload = { user_hubisoccer_id, club_depart, club_arrivee, date_transfert, type_transfert, montant, status };
        if (id) {
            const { error } = await supabaseClient.from('supabaseAuthPrive_transfers').update(payload).eq('id', id);
            if (error) throw error;
            showToast('Transfert mis à jour', 'success');
        } else {
            const { error } = await supabaseClient.from('supabaseAuthPrive_transfers').insert([payload]);
            if (error) throw error;
...
async function deleteTransfer(id) {
    showLoader(true);
    try {
        const { error } = await supabaseClient.from('supabaseAuthPrive_transfers').delete().eq('id', id);
        if (error) throw error;
...
document.getElementById('offerForm').addEventListener('submit', async (e) => {
    ...
    if (!user_hubisoccer_id) { showToast('Footballeur requis', 'warning'); return; }
    showLoader(true);
    try {
        const payload = { user_hubisoccer_id, title, from_entity, type, amount, description, status };
        if (id) {
            const { error } = await supabaseClient.from('supabaseAuthPrive_offers').update(payload).eq('id', id);
            if (error) throw error;
            showToast('Offre mise à jour', 'success');
        } else {
            const { error } = await supabaseClient.from('supabaseAuthPrive_offers').insert([payload]);
            if (error) throw error;
...
async function deleteOffer(id) {
    showLoader(true);
    try {
        const { error } = await supabaseClient.from('supabaseAuthPrive_offers').delete().eq('id', id);
        if (error) throw error;
...
function executeAction() {
    ...
    if (currentAction.type === 'deleteTransfer') deleteTransfer(currentAction.id);
    else if (currentAction.type === 'deleteOffer') deleteOffer(currentAction.id);
}
// Fin confirm modal
...
function exportData(format) {
    ...
    const rows = data.map(d => {
        if (currentTab === 'transfers') return { Footballeur: d.footballeur_name, Départ: d.club_depart, Arrivée: d.club_arrivee, Date: d.date_transfert, Type: d.type_transfert, Montant: d.montant, Statut: d.status };
        else return { Footballeur: d.footballeur_name, Titre: d.title, Entité: d.from_entity, Type: d.type, Montant: d.amount, Statut: d.status };
    });
    if (format === 'csv') {
        const csv = [Object.keys(rows[0]||{}).join(','), ...rows.map(r => Object.values(r).map(v => `"${v}"`).join(','))].join('\n');
        const blob = new Blob([csv], { type: 'text/csv' }); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `${currentTab}.csv`; a.click();
    } else if (format === 'xlsx') {
        const wb = XLSX.utils.book_new(); const ws = XLSX.utils.json_to_sheet(rows); XLSX.utils.book_append_sheet(wb, ws, currentTab); XLSX.writeFile(wb, `${currentTab}.xlsx`);
    }
}
// Fin export
...
document.getElementById('refreshBtn')?.addEventListener('click', () => { if (currentTab === 'transfers') loadTransfers(); else loadOffers(); });
