/* ============================================================
   HubISoccer -- gt-verif.js
   Page Verification du Statut - Espace Gestionnaire de Tournoi
   ------------------------------------------------------------
   Convention tables : supabaseAuthPrive_[espace]_[page]
   - supabaseAuthPrive_profiles              -> partagee (lecture)
   - supabaseAuthPrive_gt_perso_scouting -> deja existante,
   ...
   - supabaseAuthPrive_gt_perso_verif    -> table de CETTE
     page (SQL : gt-verif-table.sql, sans RLS)
   ------------------------------------------------------------
   5 documents requis -- pas d'extrait de casier judiciaire ici,
   contrairement au staff medical : l'autorite d'un organisateur pendant
   un match n'est pas de meme nature que la garde de decisions de
   sante sur un mineur.
   ============================================================ */
'use strict';
...
const VERIF_TABLE    = 'supabaseAuthPrive_gt_perso_verif';
const DOCS_BUCKET    = 'gt-perso-documents';

/* ---------- 3. DOCUMENTS REQUIS ---------- */
const DOCUMENTS_REQUIS = [
    { id: 'identite_doc', label: 'Piece d\'identite' },
    { id: 'photo_doc',    label: 'Photo d\'identite' },
    { id: 'diplome_doc',  label: 'Diplome / Certification en gestion evenementielle' },
    { id: 'ordre_doc',    label: 'Justificatif d\'affiliation Federation / Ligue' },
    { id: 'assurance_doc',label: 'Attestation d\'assurance Responsabilite Civile Organisateur' }
];
...
/* ---------- 9. CHARGEMENT PROFIL GESTIONNAIRE ---------- */
async function loadProfile() {
    ...
    gestionnaireProfile = data;
    setText('userName', gestionnaireProfile.full_name || 'Gestionnaire');
    updateNavbarAvatar();
...
function updateNavbarAvatar() {
    ...
    const url = gestionnaireProfile?.avatar_url;
    if (url && url !== '') {
        if (ui) { ui.src = url; ui.style.display = 'block'; }
        if (un) { un.style.display = 'none'; }
    } else {
        const init = getInitials(gestionnaireProfile?.full_name || 'M');
...
async function loadScoutingData() {
    if (!gestionnaireProfile) { return; }
    const { data, error } = await supabaseClient
        .from(SCOUTING_TABLE)
        .select('*')
        .eq('gestionnaire_id', gestionnaireProfile.hubisoccer_id)
        .maybeSingle();
    ...
    const { data: created } = await supabaseClient
        .from(SCOUTING_TABLE)
        .insert([{ gestionnaire_id: gestionnaireProfile.hubisoccer_id }])
        .select()
        .single();
...
async function loadOrCreateVerifRecord() {
    ...
    if (error) {
        ...
        showToast('Table de verification absente. Executez le script SQL <b>gt-verif-table.sql</b> dans Supabase.', 'warning');
        verifData = null;
        return;
    }
    ...
    const { data: created, error: createError } = await supabaseClient
        .from(VERIF_TABLE)
        .insert([{ gestionnaire_id: gestionnaireProfile.hubisoccer_id, documents: [] }])
        .select()
        .single();
    hideLoader();
...
function populateLicenceForm() {
    ...
    document.getElementById('specialite').value = scoutingData.type_organisation || '';
    document.getElementById('numOrdre').value = scoutingData.numero_licence || '';
    document.getElementById('structure').value = scoutingData.federation_affiliee || '';
    document.getElementById('dateDebut').value = scoutingData.date_debut_activite ? scoutingData.date_debut_activite.substring(0, 10) : '';
    document.getElementById('assuranceRcPro').value = scoutingData.zone_intervention || '';
    document.getElementById('paysExercice').value = scoutingData.pays || '';
}
...
async function enregistrerInfosLicence() {
    ...
    if (!specialite) { showToast('Le type d\'organisation est obligatoire.', 'warning'); return; }
    if (!numOrdre) { showToast('Le numéro de licence est obligatoire.', 'warning'); return; }
    if (!dateDebut) { showToast('La date de début d\'activité est obligatoire.', 'warning'); return; }

    const payload = {
        type_organisation     : specialite,
        numero_licence              : numOrdre,
        federation_affiliee          : (document.getElementById('structure')?.value || '').trim() || null,
        date_debut_activite              : dateDebut,
        zone_intervention                   : (document.getElementById('assuranceRcPro')?.value || '').trim() || null,
        pays                                    : (document.getElementById('paysExercice')?.value || '').trim() || null
    };
...
async function uploaderDocument(docId, docLabel, file) {
    ...
    try {
        ...
        const filePath = gestionnaireProfile.hubisoccer_id + '/' + fileName;
...
function updateCard() {
    setText('cardNomRecto', gestionnaireProfile?.full_name || '—');
    setText('cardIdRecto', 'ID : ' + (gestionnaireProfile?.hubisoccer_id || '—'));
    ...
    setText('cardSpeVerso', scoutingData?.type_organisation || '—');
    setText('cardOrdVerso', scoutingData?.numero_licence || '—');
    setText('cardDatVerso', scoutingData?.date_debut_activite ? new Date(scoutingData.date_debut_activite).toLocaleDateString('fr-FR') : '—');
    setText('cardStrVerso', scoutingData?.federation_affiliee || '—');
}
...
async function soumettreVerification() {
    if (!scoutingData || !scoutingData.type_organisation || !scoutingData.numero_licence) {
    ...
    try {
        ...
        const signaturePath = gestionnaireProfile.hubisoccer_id + '/signature_' + Date.now() + '.png';
