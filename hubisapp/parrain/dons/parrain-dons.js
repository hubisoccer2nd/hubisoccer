/* ============================================================
   HubISoccer — parrain-dons.js
   Dashboard principal du HubIS Wallet – Parrain
   ============================================================ */
...
const DON_TABLE = 'supabaseAuthPrive_parrain_data';
const DON_FK = 'parrain_id';
// Fin état global
...
function updateNavbarAvatar() {
    ...
    if (userName) userName.textContent = userProfile.full_name || userProfile.display_name || 'Parrain';
...
async function loadWallet() {
    ...
    if (!data || error) {
        ...
        setTimeout(function() { window.location.href = 'parrain-dons-setup.html'; }, 1500);
        return null;
    }
...
// Début fonctions notifications (inchangées sauf durée toast)
...
async function renderNotificationList() { /* identique aux autres pages */ }
...
// Début fonctions PIN (inchangées)
...
// Début fonction loadDonEntries
async function loadDonEntries() {
    if (!userProfile) return;
    const { data, error } = await supabaseClient.from(DON_TABLE).select('*').eq(DON_FK, userProfile.hubisoccer_id).order('created_at', { ascending: false });
    if (error) { showToast('Erreur chargement dons', 'error'); return; }
    allDonEntries = data || [];
    renderDonEntries();
    updateDonStats();
}
// Fin fonction loadDonEntries

// Début fonction renderDonEntries
function renderDonEntries() {
    const searchTerm = (document.getElementById('searchInputDon')?.value || '').toLowerCase();
    const filterType = document.getElementById('filterSelect')?.value || '';
    const filtered = allDonEntries.filter(function(e) {
        const txt = JSON.stringify(e).toLowerCase();
        return (!searchTerm || txt.includes(searchTerm)) && (!filterType || (e.type_don === filterType));
    });
    const grid = document.getElementById('entriesGrid');
    if (!grid) return;
    grid.innerHTML = '';
    if (!filtered.length) { grid.innerHTML = '<div class="empty-state"><i class="fas fa-hand-holding-usd"></i><p>Aucun résultat.</p></div>'; return; }
    filtered.forEach(function(item) {
        const card = document.createElement('div');
        card.className = 'entry-card';
        const dateStr = item.created_at ? new Date(item.created_at).toLocaleDateString('fr-FR') : '';
        const badgeVal = item.type_don || '';
        let meta = '';
        if (item.type_don) meta += '<span><i class="fas fa-info-circle"></i>' + item.type_don + '</span>';
        if (dateStr) meta += '<span><i class="fas fa-calendar-alt"></i>' + dateStr + '</span>';
        card.innerHTML = '<div class="entry-card-header"><span class="entry-card-title">' + (item.nom_beneficiaire || 'Sans titre') + '</span>' + (badgeVal ? '<span class="entry-badge">' + badgeVal + '</span>' : '') + '</div><div class="entry-meta">' + meta + '</div><div class="entry-actions"><button class="btn-edit" onclick="openEditDon(\'' + item.id + '\')"><i class="fas fa-edit"></i> Modifier</button><button class="btn-del" onclick="deleteDonEntry(\'' + item.id + '\')"><i class="fas fa-trash"></i> Supprimer</button></div>';
        grid.appendChild(card);
    });
}
// Fin fonction renderDonEntries

// Début fonction updateDonStats
function updateDonStats() {
    document.getElementById('statTotal').textContent = allDonEntries.length;
    const now = new Date(), m = now.getMonth(), y = now.getFullYear();
    const mois = allDonEntries.filter(function(e) { if (!e.created_at) return false; const d = new Date(e.created_at); return d.getMonth() === m && d.getFullYear() === y; }).length;
    document.getElementById('statMois').textContent = mois;
    document.getElementById('statActifs').textContent = allDonEntries.length;
    const last = allDonEntries[0];
    document.getElementById('statLast').textContent = last ? (last.nom_beneficiaire || '—').substring(0, 14) : '—';
}
// Fin fonction updateDonStats

// Début fonction openAddDon
function openAddDon() {
    editingDonId = null;
    document.getElementById('modalTitle').innerHTML = '<i class="fas fa-plus"></i> Ajouter — Mes Dons & Bourses';
    document.getElementById('entryForm').reset();
    document.getElementById('f__id').value = '';
    document.getElementById('entryModal').classList.add('show');
}
// Fin fonction openAddDon

// Début fonction openEditDon
function openEditDon(id) {
    const item = allDonEntries.find(function(e) { return e.id === id; });
    if (!item) return;
    editingDonId = id;
    document.getElementById('modalTitle').innerHTML = '<i class="fas fa-edit"></i> Modifier';
    document.getElementById('f__id').value = id;
    document.getElementById('f_nom_beneficiaire').value = item.nom_beneficiaire || '';
    document.getElementById('f_type_don').value = item.type_don || '';
    document.getElementById('f_montant_don').value = item.montant_don || '';
    document.getElementById('f_date_don').value = item.date_don || '';
    document.getElementById('f_objet_don').value = item.objet_don || '';
    document.getElementById('f_statut_don').value = item.statut_don || '';
    document.getElementById('f_preuve_don').value = item.preuve_don || '';
    document.getElementById('f_notes_don').value = item.notes_don || '';
    document.getElementById('entryModal').classList.add('show');
}
window.openEditDon = openEditDon;

// Début fonction saveDonEntry
async function saveDonEntry() {
    if (!userProfile) return;
    const data = {
        nom_beneficiaire: document.getElementById('f_nom_beneficiaire').value,
        type_don: document.getElementById('f_type_don').value,
        montant_don: document.getElementById('f_montant_don').value,
        date_don: document.getElementById('f_date_don').value,
        objet_don: document.getElementById('f_objet_don').value,
        statut_don: document.getElementById('f_statut_don').value,
        preuve_don: document.getElementById('f_preuve_don').value,
        notes_don: document.getElementById('f_notes_don').value
    };
    data[DON_FK] = userProfile.hubisoccer_id;
    data.updated_at = new Date().toISOString();
    showLoader();
    let error;
    if (editingDonId) {
        ({ error } = await supabaseClient.from(DON_TABLE).update(data).eq('id', editingDonId));
    } else {
        data.created_at = new Date().toISOString();
        ({ error } = await supabaseClient.from(DON_TABLE).insert([data]).select().single());
    }
    hideLoader();
    if (error) { showToast('Erreur : ' + error.message, 'error'); return; }
    showToast(editingDonId ? 'Modifié !' : 'Ajouté !', 'success');
    document.getElementById('entryModal').classList.remove('show');
    await loadDonEntries();
}
// Fin fonction saveDonEntry

// Début fonction deleteDonEntry
async function deleteDonEntry(id) {
    if (!confirm('Supprimer cette entrée ?')) return;
    showLoader();
    const { error } = await supabaseClient.from(DON_TABLE).delete().eq('id', id);
    hideLoader();
    if (error) { showToast('Erreur', 'error'); return; }
    showToast('Entrée supprimée', 'info');
    allDonEntries = allDonEntries.filter(function(e) { return e.id !== id; });
    renderDonEntries();
    updateDonStats();
}
window.deleteDonEntry = deleteDonEntry;

// Début fonction executeDon (don via wallet)
async function executeDon() {
    if (!walletData) return;
    const recipientId = document.getElementById('donRecipientId')?.value?.trim().toUpperCase();
    const amountVal = parseFloat(document.getElementById('donAmount')?.value);
    const donType = document.getElementById('donType')?.value || 'don';
    const message = document.getElementById('donMessage')?.value?.trim() || '';
    const pin = getPinValue('donpin');
    if (!recipientId) { showToast('Veuillez saisir un Wallet ID ou HID destinataire', 'warning'); return; }
    if (!amountVal || amountVal < 100) { showToast('Montant minimum : 100 CFA', 'warning'); return; }
    if (amountVal > (walletData.balance || 0)) { showToast('Solde insuffisant', 'error'); return; }
    if (pin.length < 6) { showToast('Saisissez votre PIN', 'warning'); return; }
    const pinOk = await verifyPin(pin);
    if (!pinOk) { showToast('PIN incorrect', 'error'); setPinError('donpin'); return; }
    const { data: recipWallet, error: rwErr } = await supabaseClient.from('supabaseAuthPrive_hubis_wallets').select('id, hubisoccer_id, balance, status').eq('wallet_ref', recipientId).maybeSingle();
    if (rwErr || !recipWallet) { showToast('Destinataire introuvable', 'error'); return; }
    if (recipWallet.status !== 'active') { showToast('Le wallet destinataire est inactif', 'error'); return; }
    showLoader();
    try {
        const ref = generateRef('DON');
        const desc = 'Don ' + (donType === 'bourse' ? 'Bourse' : '') + ' – ' + message;
        await supabaseClient.from('supabaseAuthPrive_hubis_transactions').insert([{ wallet_id: walletData.id, type: 'transfer_out', amount: amountVal, description: desc, reference: ref, recipient_ref: recipientId, sender_ref: walletData.wallet_ref, status: 'completed', created_at: new Date().toISOString() }]);
        await supabaseClient.from('supabaseAuthPrive_hubis_wallets').update({ balance: walletData.balance - amountVal, updated_at: new Date().toISOString() }).eq('id', walletData.id);
        await supabaseClient.from('supabaseAuthPrive_hubis_transactions').insert([{ wallet_id: recipWallet.id, type: 'transfer_in', amount: amountVal, description: desc, reference: ref, sender_ref: walletData.wallet_ref, recipient_ref: recipientId, status: 'completed', created_at: new Date().toISOString() }]);
        await supabaseClient.from('supabaseAuthPrive_hubis_wallets').update({ balance: recipWallet.balance + amountVal, updated_at: new Date().toISOString() }).eq('id', recipWallet.id);
        // Enregistrer dans la table des dons
        await supabaseClient.from(DON_TABLE).insert([{ parrain_id: userProfile.hubisoccer_id, nom_beneficiaire: recipientId, type_don: donType, montant_don: amountVal, statut_don: 'Versé', date_don: new Date().toISOString().split('T')[0], notes_don: message, created_at: new Date().toISOString() }]);
        // Notification au destinataire
        await supabaseClient.from('supabaseAuthPrive_notifications').insert([{ recipient_hubisoccer_id: recipWallet.hubisoccer_id, type: 'don_received', title: '🤝 Don reçu !', message: 'Vous avez reçu un don de ' + formatAmount(amountVal) + ' CFA.', read: false, created_at: new Date().toISOString() }]);
        hideLoader(); closeModal('newDonModal');
        showToast('Don de ' + formatAmount(amountVal) + ' CFA effectué avec succès !', 'success');
        walletData.balance -= amountVal;
        renderBalance();
        await loadTransactions(); await computeMonthStats();
        await loadDonEntries();
    } catch (err) { console.error('Don error:', err); showToast('Erreur lors du don', 'error'); hideLoader(); }
}
// Fin fonction executeDon
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    window.executeDon = executeDon;

    // Événements Dons & Bourses
    document.getElementById('btnAddDon')?.addEventListener('click', openAddDon);
    ...
    document.getElementById('modalSave')?.addEventListener('click', saveDonEntry);
    document.getElementById('entryModal')?.addEventListener('click', function(e) { if (e.target === this) this.classList.remove('show'); });
    document.getElementById('searchInputDon')?.addEventListener('input', renderDonEntries);
    document.getElementById('filterSelect')?.addEventListener('change', renderDonEntries);
});
