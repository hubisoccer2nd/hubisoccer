/* ============================================================
   HubISoccer -- agent-profile.js
   Page Mon Profil - Espace Parrain
   Inspire de foot-profile.html : memes sections (identite,
   infos personnelles, zone dangereuse) -- sections "Etudes &
   Formations" et "Carriere & Metier" retirees (redondantes avec
   Mon CV Pro), bouton "Certifier" retire (redondant avec
...
function populateForm() {
    ...
    document.getElementById('country').value = p.country_code || '';
}
...
async function saveProfile(e) {
    ...
    Object.assign(parrainProfile, updates);
}
...
function updateDeletionUI() {
    ...
    if (parrainProfile.deletion_requested && !parrainProfile.deletion_revoked) {
        if (delBtn) { delBtn.style.display = 'none'; }
        if (revokeBtn) { revokeBtn.style.display = 'inline-flex'; }
        if (status && parrainProfile.deletion_requested_at) {
            const reqDate = new Date(parrainProfile.deletion_requested_at);
...
async function confirmerSuppression() {
    ...
    if (error) { showToast('Erreur : ' + error.message, 'error'); return; }
    parrainProfile.deletion_requested = true;
    parrainProfile.deletion_requested_at = new Date().toISOString();
    parrainProfile.deletion_revoked = false;
...
async function revokeDeletion() {
    ...
    if (error) { showToast('Erreur : ' + error.message, 'error'); return; }
    parrainProfile.deletion_requested = false;
    parrainProfile.deletion_revoked = true;
    updateDeletionUI();
