/* ============================================================
   HubISoccer -- parrain-scouting.js
   Page Scouting - Espace Parrain
   ------------------------------------------------------------
   ...
   donnees fausses.
   ------------------------------------------------------------
   "Parrainer ce talent" envoie vers Mes Proteges avec nom/prenom/
   role pre-remplis (?nom=&prenom=&role=) -- Mes Proteges est une
   fiche libre sans lien direct vers un compte, donc pas de
   recherche par ID ici, juste un pre-remplissage du formulaire.
   ============================================================ */
'use strict';
...
function parrainerTalent() {
    if (!selectedProfile) { return; }
    const parts = (selectedProfile.full_name || '').trim().split(/\s+/);
    const prenom = parts[0] || '';
    const nom = parts.slice(1).join(' ') || '';
    const params = new URLSearchParams({
        nom   : nom,
        prenom : prenom,
        role    : selectedProfile.position || ''
    });
    window.location.href = '../proteges/parrain-proteges.html?' + params.toString();
}
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    document.getElementById('btnParrainerTalent')?.addEventListener('click', parrainerTalent);
