/* ============================================================
   HubISoccer -- parrain-settings.js
   Page Parametres - Espace Parrain
...
async function saveSettings(e) {
    ...
    else { showToast('Paramètres enregistrés. ✅', 'success'); Object.assign(parrainProfile, updates); }
}
...
function exportData() {
    const dataStr = JSON.stringify(parrainProfile, null, 2);
    const blob = new Blob([dataStr], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'hubisoccer_' + (parrainProfile.hubisoccer_id || 'export') + '.json';
    a.click();
