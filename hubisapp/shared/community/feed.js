...
// ═══════════════════════════════════════════════════════════
// LOT B — RAFRAÎCHIR LE FIL
// -----------------------------------------------------------
// Trois boutons « rafraîchir » existaient : les suggestions, les
// présences, les suggestions du profil. Aucun ne rechargeait LES
// PUBLICATIONS. Pour voir ce qui venait d'être publié, il fallait
// recharger la page entière — ce qui fait repartir tout en haut,
// vide les commentaires ouverts, et refait tomber toute la page
// sur une connexion mobile.
//
// Ici, on ne recharge QUE le fil, et on le dit.
// ═══════════════════════════════════════════════════════════
let _rafraichissementEnCours = false;

async function rafraichirLeFil() {
    if (_rafraichissementEnCours) return;          // double clic : un seul travail
    _rafraichissementEnCours = true;

    const bouton = document.getElementById('refreshFeedBtn');
    const icone = bouton ? bouton.querySelector('i') : null;
    if (icone) icone.classList.add('tourne');
    if (bouton) bouton.disabled = true;

    const combienAvant = posts.length;

    try {
        // On repart du début : postOffset est remis à zéro par
        // loadPosts(true), sinon on rechargerait la page suivante.
        postOffset = 0;
        await loadPosts(true);

        const nouveaux = posts.length - combienAvant;
        toast(nouveaux > 0
                ? nouveaux + ' nouvelle(s) publication(s).'
                : 'Le fil est à jour.',
              nouveaux > 0 ? 'success' : 'info');

        // Les stories aussi : elles expirent en 24 h, c'est ce qui
        // se périme le plus vite de toute la page.
        if (typeof loadStories === 'function') {
            try { await loadStories(); } catch (e) { /* déjà signalé par loadStories */ }
        }
    } catch (err) {
        // Un rafraîchissement qui échoue en silence laisserait
        // croire que rien de neuf n'a été publié.
        const raison = (typeof describeDbError === 'function') ? describeDbError(err) : (err.message || '');
        toast('Rafraîchissement impossible — ' + raison, 'error');
    } finally {
        _rafraichissementEnCours = false;
        if (icone) icone.classList.remove('tourne');
...
async function loadPosts(reset = false) {
    ...
    try {
        ...
        // ============================================================
        // LOT A — la détection passe par HubJointure, qui reconnaît
        // AUSSI le 42703 citant la jointure dans le nom de colonne.
        // Ce repli-ci ne voyait que PGRST200/201 et laissait donc
        // passer une panne qu'il savait pourtant réparer.
        if (error && (typeof HubJointure !== 'undefined'
                        ? HubJointure.estUnProblemeDeJointure(error)
                        : (error.code === 'PGRST201' || error.code === 'PGRST200'))) {
...
function makePostCard(post) {
    ...
    if (post.media_url) {
        if (post.media_type === 'video') {
            mediaHtml = `<div class="post-media"><video src="${escapeAttr(post.media_url)}" class="hub-lecteur" data-dans-le-fil="1" controls preload="metadata" playsinline></video></div>`;
        } else {
    ...
    if (post.event_data) {
        ...
        // CHANTIER 18 — une publication venue du gestionnaire de
        // tournoi porte un bloc « hubisoccer » dans son
        // event_data. Elle merite sa propre carte : un score, un
        // tour franchi, un champion, et un bouton qui ouvre la
        // page publique du tournoi.
        //
        // carteDeTournoiHtml() rend une chaine VIDE pour tout
        // event_data ordinaire : une publication d'evenement
        // classique garde exactement la carte qu'elle avait, au
        // pixel pres. Et si utils.js n'a pas encore ete mis a
        // jour, on retombe sur l'ancienne carte sans rien casser.
        const carteTournoi = (typeof carteDeTournoiHtml === 'function')
            ? carteDeTournoiHtml(evt) : '';

        if (carteTournoi) {
            eventHtml = carteTournoi;
        } else {
...
async function publishDueScheduledPosts() {
    ...
    try {
        ...
        // LOT A — une publication programmée qui ne part pas, et
        // personne ne le sait : l'auteur croit son message publié.
        console.warn('Posts programmés :', e);
        const raison = (typeof describeDbError === 'function') ? describeDbError(e) : (e.message || '');
        toast('Tes publications programmées n\'ont pas pu être publiées — ' + raison +
              '. Elles restent en attente : réessaie en rouvrant le fil.', 'error');
    }
}
...
async function loadComments(postId) {
    ...
    // LOT A — filet sous la jointure.
    const { data, error } = await HubJointure.avecFilet(sb, {
        requete: () => sb
        ...
        requeteSansJointure: () => sb
            ...
            .order('created_at', { ascending: true })
            .limit(10),
        liens: [HubJointure.profil('author', 'author_hubisoccer_id', 'hubisoccer_id, full_name, display_name, avatar_url, role_code')]
    });
...
async function sendComment(postId) {
    ...
    try {
        ...
        const { data, error } = await sb.from('supabaseAuthPrive_comments').insert({
            ...
            audio_url: audioUrl,
            parent_id: null
        // LOT A — PAS de filet ici : rejouer une INSERTION
        // créerait un deuxième commentaire. On insère sans
        // jointure et on rattache l'auteur, qui est nous.
        }).select('*').single();

        if (error) throw error;
        if (data) data.author = currentProfile;
...
async function sendReply(commentId = null, postId = null) {
    ...
    try {
        const { data, error } = await sb.from('supabaseAuthPrive_comments').insert({
            ...
            parent_id: id
        // LOT A — PAS de filet ici : rejouer une INSERTION
        // créerait un deuxième réponse. On insère sans
        // jointure et on rattache l'auteur, qui est nous.
        }).select('*').single();
...
function openMediaModal(url, type, postId) {
    ...
    viewer.innerHTML = type === 'video'
        ? `<video src="${escapeAttr(url)}" class="hub-lecteur" controls autoplay playsinline style="max-width:90vw;max-height:80vh;border-radius:8px"></video>`
...
async function loadStories() {
    try {
        ...
        if (followingIds.length > 0) {
            const maintenantStories = new Date().toISOString();
            const { data } = await HubJointure.avecFilet(sb, {
                requete: () => sb.from('supabaseAuthPrive_stories')
                    ...
                    .gt('expires_at', maintenantStories)
                    .order('created_at', { ascending: false })
                    .limit(5),
                requeteSansJointure: () => sb.from('supabaseAuthPrive_stories')
                    .select('*')
                    .in('user_hubisoccer_id', followingIds)
                    .gt('expires_at', maintenantStories)
                    .order('created_at', { ascending: false })
                    .limit(5),
                liens: [HubJointure.profil('author', 'user_hubisoccer_id', 'hubisoccer_id, full_name, display_name, avatar_url')]
            });
        ...
        // LOT A — la barre de stories restait vide sans un mot :
        // on croyait que personne n'avait publié.
        console.warn('Erreur stories:', err);
        const barre = document.getElementById('storiesBar') || document.getElementById('storiesList');
        const raison = (typeof describeDbError === 'function') ? describeDbError(err) : (err.message || '');
        if (barre) {
            barre.innerHTML = '<p style="font-size:0.78rem;color:var(--danger);padding:8px 12px;">' +
                'Stories non chargées — ' + escapeHtml(raison) + '</p>';
        } else {
            toast('Stories non chargées — ' + raison, 'error');
        }
    }
}
...
function renderStoryViewer() {
    ...
    if (story.media_type === 'text') {
        ...
        media.innerHTML = `<video src="${escapeAttr(story.media_url)}" class="hub-lecteur" autoplay playsinline controls></video>`;
    ...
    if (preview) {
        preview.innerHTML = `
            <div style="position:relative">
                ${isVideo ? `<video src="${escapeAttr(url)}" class="hub-lecteur" controls playsinline style="width:100%;max-height:240px;border-radius:8px"></video>` : `<img src="${escapeAttr(url)}" style="width:100%;max-height:240px;object-fit:cover;border-radius:8px">`}
...
async function loadLives() {
    ...
    try {
        // LOT A — « host:host_hubisoccer_id(...) » ne nomme pas la
        // table : PostgREST doit deviner la relation. C'est la forme
        // la plus fragile du module.
        const { data } = await HubJointure.avecFilet(sb, {
            requete: () => sb
            ...
            requeteSansJointure: () => sb
                ...
                .order('started_at', { ascending: false })
                .limit(5),
            liens: [HubJointure.profil('host', 'host_hubisoccer_id',
                                       'hubisoccer_id, full_name, display_name, avatar_url')]
        });
...
async function loadSuggestions() {
    ...
    // LOT A — la fabrique, pour que le filtre s'applique aux deux.
    const faireSuggestions = (avecJointure) => {
        let r = sb.from('supabaseAuthPrive_communities')
            .select(avecJointure
                ? '*, profiles:supabaseAuthPrive_profiles!hubisoccer_id(role_code, certified)'
                : '*');
        if (exclude.length) r = r.not('hubisoccer_id', 'in', `(${exclude.join(',')})`);
        return r;
    };
    // CHANTIER 10 — cinq suggestions, c'etait trop court : la
    // colonne se vidait des qu'on suivait quelques personnes.
    // LOT A — et le filet, pour que la colonne ne se vide plus
    // non plus quand la clé étrangère manque.
    const { data } = await HubJointure.avecFilet(sb, {
        requete: () => faireSuggestions(true).limit(12),
        requeteSansJointure: () => faireSuggestions(false).limit(12),
        liens: [HubJointure.profil('profiles', 'hubisoccer_id',
                                   'hubisoccer_id, role_code, certified')]
    });
...
async function loadFollowers() {
    ...
    try {
        ...
        // LOT A — « Pas encore d'abonnés » était un MENSONGE :
        // la personne en a peut-être des centaines, c'est la
        // lecture qui a échoué. On ne confond plus « zéro » et
        // « je n'ai pas réussi à compter ».
        console.warn('Erreur chargement followers:', err);
        const raison = (typeof describeDbError === 'function') ? describeDbError(err) : (err.message || '');
        container.innerHTML = '<p style="font-size:0.78rem;color:var(--danger);">' +
            'Abonnés non chargés — ' + escapeHtml(raison) + '</p>';
    }
}
    ...
    try {
        const { data } = await HubJointure.avecFilet(sb, {
            ...
            requeteSansJointure: () => sb
                ...
                .select('blocked_hubisoccer_id')
                .eq('user_hubisoccer_id', currentProfile.hubisoccer_id),
            liens: [HubJointure.profil('blocked', 'blocked_hubisoccer_id',
                                       'hubisoccer_id, full_name, display_name, avatar_url, feed_id')]
        });
...
async function loadHiddenPosts() {
    const { data } = await HubJointure.avecFilet(sb, {
        requete: () => sb.from('supabaseAuthPrive_hidden_posts')
            .select('post_id, post:supabaseAuthPrive_posts!post_id(content, created_at)')
            .eq('user_hubisoccer_id', currentProfile.hubisoccer_id)
            .order('created_at', { ascending: false }),
        requeteSansJointure: () => sb.from('supabaseAuthPrive_hidden_posts')
            .select('post_id, created_at')
            .eq('user_hubisoccer_id', currentProfile.hubisoccer_id)
            .order('created_at', { ascending: false }),
        liens: [HubJointure.vers('post', 'supabaseAuthPrive_posts', 'post_id', 'id',
...
async function loadCollections() {
    const { data } = await HubJointure.avecFilet(sb, {
        requete: () => sb.from('supabaseAuthPrive_saved_posts')
            .select('post_id, post:supabaseAuthPrive_posts!post_id(content, created_at)')
            .eq('user_hubisoccer_id', currentProfile.hubisoccer_id)
            .order('created_at', { ascending: false }),
        requeteSansJointure: () => sb.from('supabaseAuthPrive_saved_posts')
            .select('post_id, created_at')
            .eq('user_hubisoccer_id', currentProfile.hubisoccer_id)
            .order('created_at', { ascending: false }),
        liens: [HubJointure.vers('post', 'supabaseAuthPrive_posts', 'post_id', 'id',
...
async function syncCommunityPostsCount() {
    ...
    try {
        ...
        // LOT A — même règle : un compteur qu'on n'a pas pu
        // vérifier porte un « ~ » au lieu de mentir.
        console.warn('[feed] recomptage des publications impossible :', err.message);
        marquerCompteurNonVerifie(document.getElementById('myCommPosts'), 'recomptage impossible');
        return null;
    }
}
...
async function publishPost() {
    ...
    try {
        ...
        // LOT A — PAS de filet : rejouer cette insertion publierait
        // DEUX FOIS le message. On insère sans jointure et on
        // rattache l'auteur, qui est nous.
        const { data: newPost, error } = await sb.from('supabaseAuthPrive_posts').insert(postData)
            .select('*')
            .single();
        if (error) throw error;
        if (newPost) newPost.author = currentProfile;
...
function showPreview() {
    ...
    if (mediaFile) {
        ...
        mediaHtml = isVideo ?
            `<div class="post-media"><video src="${escapeAttr(url)}" class="hub-lecteur" data-dans-le-fil="1" controls playsinline></video></div>` :
...
async function init() {
    ...
    document.getElementById('mediaInput').addEventListener('change', (e) => {
        ...
        document.getElementById('mediaPreview').innerHTML = `
            <div class="preview-media-wrap" style="position:relative">
                ${isVideo ? `<video src="${escapeAttr(url)}" class="hub-lecteur" controls playsinline></video>` : `<img src="${escapeAttr(url)}" alt="">`}
    ...
    // LOT B — le bouton « Rafraîchir ».
    //
    // Il porte sa PROPRE classe, et pas « feed-filter-btn » :
    // sinon le gestionnaire ci-dessus l'aurait traité comme un
    // filtre, avec un data-filter indéfini — et le fil se serait
    // vidé au lieu de se recharger.
    document.getElementById('refreshFeedBtn')?.addEventListener('click', rafraichirLeFil);
