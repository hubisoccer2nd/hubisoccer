// ============================================================
//  HUBISOCCER — LIVE.JS (VERSION CORRIGÉE)
//  Lives en direct — PeerJS Cloud + Supabase Realtime
// ============================================================
...
async function initSessionAndProfile() {
    ...
    updateAvatarDisplay(currentProfile.avatar_url, currentProfile.full_name || currentProfile.display_name, 'liveUser');
    updateAvatarDisplay(currentProfile.avatar_url, currentProfile.full_name || currentProfile.display_name, 'chatUser');
...
async function loadLives() {
    // LOT A — filet sous la jointure. Sans clé étrangère déclarée,
    // PostgREST refusait la requête ENTIÈRE et la grille des lives
    // devenait blanche, sans un mot.
...
async function loadPastLives() {
    const { data } = await HubJointure.avecFilet(sb, {
        requete: () => sb
            .from('supabaseAuthPrive_live_sessions')
            .select('*, host:supabaseAuthPrive_profiles!host_hubisoccer_id(full_name, display_name)')
            .eq('is_active', false)
        ...
        requeteSansJointure: () => sb
            ...
            .limit(8),
        liens: [HubJointure.profil('host', 'host_hubisoccer_id',
                                   'hubisoccer_id, full_name, display_name')]
    });
...
async function init() {
    ...
    if (roomId) {
        const { data } = await HubJointure.avecFilet(sb, {
            requete: () => sb.from('supabaseAuthPrive_live_sessions')
                .select('*, host:supabaseAuthPrive_profiles!host_hubisoccer_id(full_name, display_name, avatar_url, role_code)')
                .eq('id', roomId).eq('is_active', true).maybeSingle(),
            requeteSansJointure: () => sb.from('supabaseAuthPrive_live_sessions')
                .select('*')
                .eq('id', roomId).eq('is_active', true).maybeSingle(),
            liens: [HubJointure.profil('host', 'host_hubisoccer_id',
                                       'hubisoccer_id, full_name, display_name, avatar_url, role_code')]
