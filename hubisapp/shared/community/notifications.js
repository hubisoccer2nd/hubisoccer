...
async function markAsRead(id) {
  // LOT B — même faute qu'au-dessus, en plus discret : l'échec
  // passait inaperçu et la notification « redevenait » non lue
  // au rechargement.
  const { error } = await sb.from('supabaseAuthPrive_notifications').update({ read: true }).eq('id', id);
  if (error) {
    const raison = (typeof describeDbError === 'function') ? describeDbError(error) : error.message;
    toast('Notification non marquée comme lue — ' + raison, 'error');
    return;
  }
...
async function deleteNotification(id) {
  // LOT B — la suppression n'était pas vérifiée : la ligne
  // disparaissait de l'écran mais restait en base, et revenait
  // au rechargement. On ne retire de l'affichage que ce qui a
  // VRAIMENT été supprimé.
  const { error } = await sb.from('supabaseAuthPrive_notifications').delete().eq('id', id);
  if (error) {
    const raison = (typeof describeDbError === 'function') ? describeDbError(error) : error.message;
    toast('Notification NON supprimée — ' + raison, 'error');
    return;
  }
...
async function markAllAsRead() {
  // LOT B — l'écriture n'était pas vérifiée.
  //
  // En cas d'échec, la page marquait quand même TOUT comme lu à
  // l'écran et annonçait la réussite en vert. Au rechargement,
  // les notifications revenaient non lues et on ne comprenait
  // pas pourquoi.
  const { error } = await sb.from('supabaseAuthPrive_notifications')
    .update({ read: true })
    .eq('recipient_hubisoccer_id', currentProfile.hubisoccer_id)
    .eq('read', false);
  ...
  if (error) {
    const raison = (typeof describeDbError === 'function') ? describeDbError(error) : error.message;
    toast('Les notifications n’ont PAS été marquées comme lues — ' + raison, 'error');
    return;
  }
...
async function clearAll() {
  ...
  // LOT B — même règle : on vide l'écran seulement si la base a
  // vraiment été vidée.
  const { error } = await sb.from('supabaseAuthPrive_notifications')
    .delete()
    .eq('recipient_hubisoccer_id', currentProfile.hubisoccer_id);
  if (error) {
    const raison = (typeof describeDbError === 'function') ? describeDbError(error) : error.message;
    toast('Les notifications n’ont PAS été supprimées — ' + raison, 'error');
    return;
  }
  notifications = [];
  renderNotifications();
