// ============================================================
//  HUBISOCCER — POST-VIEW.JS (VERSION FINALE CORRIGÉE)
// ============================================================
...
async function loadPost(postId) {
    // LOT A — filet sous la jointure : sans lui, une clé étrangère
    // manquante et la publication entière devenait introuvable.
    const { data, error } = await HubJointure.avecFilet(sb, {
        requete: () => sb
            .from('supabaseAuthPrive_posts')
            .select(`
                *,
                author:supabaseAuthPrive_profiles!author_hubisoccer_id(hubisoccer_id, full_name, display_name, avatar_url, role_code, feed_id, certified, bio),
                community:supabaseAuthPrive_communities!community_id(name, feed_id, avatar_url, followers_count, posts_count)
            `)
            .eq('id', postId)
            .maybeSingle(),
        requeteSansJointure: () => sb
            ...
            .maybeSingle(),
        liens: [
            HubJointure.profil('author', 'author_hubisoccer_id',
                'hubisoccer_id, full_name, display_name, avatar_url, role_code, feed_id, certified, bio'),
            HubJointure.vers('community', 'supabaseAuthPrive_communities', 'community_id', 'id',
                'id, name, feed_id, avatar_url, followers_count, posts_count')
        ]
    });
...
function renderPost(post) {
    ...
    if (post.event_data) {
        ...
        // CHANTIER 18 — meme carte que dans le fil, pour qu'une
        // publication de tournoi ouverte en pleine page ne perde
        // pas son bouton « Suivre le tournoi ».
...
async function refreshPostCount(table, column, elementId) {
    ...
    if (total === null) {
        // LOT A — le recomptage a échoué : le chiffre affiché est
        // l'estimation optimiste, pas la vérité. On le dit.
        marquerCompteurNonVerifie(document.getElementById(elementId), 'recomptage impossible');
        return null;
    }
...
async function loadComments(append = false) {
    ...
    // LOT A — la fabrique construit les deux versions, pour que
    // le tri s'applique aux deux de la même façon.
    const faireCommentaires = (avecJointure) => {
        let r = sb.from('supabaseAuthPrive_comments')
            .select(avecJointure
                ? '*, author:supabaseAuthPrive_profiles!author_hubisoccer_id(full_name, display_name, avatar_url, role_code)'
                : '*')
            .eq('post_id', currentPost.id)
            .is('parent_id', null)
            .range(commentOffset, commentOffset + COMMENT_PAGE - 1);
        if (sort === 'popular') r = r.order('likes_count', { ascending: false });
        else                    r = r.order('created_at', { ascending: true });
        return r;
    };
    let query = faireCommentaires(true);
    ...
    for (const c of data || []) {
        const { data: replies } = await HubJointure.avecFilet(sb, {
            requete: () => sb.from('supabaseAuthPrive_comments')
                .select('*, author:supabaseAuthPrive_profiles!author_hubisoccer_id(full_name, display_name, avatar_url)')
                .eq('parent_id', c.id)
                .order('created_at', { ascending: true })
                .limit(3),
            requeteSansJointure: () => sb.from('supabaseAuthPrive_comments')
                .select('*')
                .eq('parent_id', c.id)
                .order('created_at', { ascending: true })
                .limit(3),
            liens: [HubJointure.profil('author', 'author_hubisoccer_id', 'hubisoccer_id, full_name, display_name, avatar_url')]
        });
...
async function sendReply(parentId) {
    ...
    const { data, error } = await sb.from('supabaseAuthPrive_comments').insert({
        ...
        content,
        parent_id: parentId
    // LOT A — ATTENTION : ici on N'UTILISE PAS avecFilet.
    //
    // Rejouer une INSERTION créerait un deuxième commentaire. On
    // insère donc sans jointure, et on rattache l'auteur — qu'on
    // connaît déjà, puisque c'est nous.
    }).select('*').single();

    if (error) { toast('Erreur envoi réponse : ' + error.message, 'error'); return; }
...
async function sendComment() {
    ...
    try {
        ...
        const { data, error } = await sb.from('supabaseAuthPrive_comments').insert({
            ...
            audio_url: audioUrl,
            parent_id: null
        // LOT A — même raison qu'au-dessus : pas de filet sur une
        // insertion, sinon le commentaire partirait en double.
        }).select('*').single();
...
function showLikes() {
    ...
    HubJointure.avecFilet(sb, {
        requete: () => sb.from('supabaseAuthPrive_post_likes')
            .select('user_hubisoccer_id, profiles:supabaseAuthPrive_profiles!user_hubisoccer_id(full_name, display_name, avatar_url)')
            .eq('post_id', currentPost.id)
            .limit(30),
        requeteSansJointure: () => sb.from('supabaseAuthPrive_post_likes')
            .select('user_hubisoccer_id')
            .eq('post_id', currentPost.id)
            .limit(30),
        liens: [HubJointure.profil('profiles', 'user_hubisoccer_id', 'hubisoccer_id, full_name, display_name, avatar_url')]
    })
        .then(({ data }) => {
...
function showDislikes() {
    ...
    HubJointure.avecFilet(sb, {
        requete: () => sb.from('supabaseAuthPrive_post_dislikes')
            .select('user_hubisoccer_id, profiles:supabaseAuthPrive_profiles!user_hubisoccer_id(full_name, display_name, avatar_url)')
            .eq('post_id', currentPost.id)
            .limit(30),
        requeteSansJointure: () => sb.from('supabaseAuthPrive_post_dislikes')
            .select('user_hubisoccer_id')
            .eq('post_id', currentPost.id)
            .limit(30),
        liens: [HubJointure.profil('profiles', 'user_hubisoccer_id', 'hubisoccer_id, full_name, display_name, avatar_url')]
...
async function init() {
    ...
    ta.addEventListener('keydown', function(e) { if (e.key === 'Enter' && e.ctrlKey) { e.preventDefault(); sendComment(); } });

    document.getElementById('sendCommentBtn').addEventListener('click', sendComment);
    document.getElementById('loadMoreCommentsBtn').addEventListener('click', function() { loadComments(true); });
    ...
    document.querySelectorAll('.share-btn-pv').forEach(function(btn) { btn.addEventListener('click', function() { sharePost(btn.dataset.network); }); });
    document.getElementById('submitReportBtn').addEventListener('click', submitReport);

    document.getElementById('userMenu').addEventListener('click', function(e) { e.stopPropagation(); document.getElementById('userDropdown').classList.toggle('show'); });
    document.addEventListener('click', function() { const dd = document.getElementById('userDropdown'); if (dd) dd.classList.remove('show'); });
    document.getElementById('dropLogout').addEventListener('click', logout);

    document.querySelectorAll('.c-modal').forEach(function(m) { m.addEventListener('click', function(e) { if (e.target === m) closeModal(m.id); }); });
}

function subscribeComments() {
    commentSubscription = sb.channel(`post_comments_${currentPost.id}`)
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'supabaseAuthPrive_comments', filter: `post_id=eq.${currentPost.id}` }, async function(payload) {
            const c = payload.new;
