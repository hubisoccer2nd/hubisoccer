...
async function loadProfileData(identifier) {
    ...
    try {
        ...
        //
        // Celle-ci utilisait la forme la plus fragile : « profiles:
        // hubisoccer_id(...) » ne NOMME PAS la table. PostgREST doit
        // deviner la relation à partir de la seule colonne — et s'il
        // n'y arrive pas, la page de profil entière reste vide.
        const { data, error } = await HubJointure.avecFilet(sb, {
            ...
            requeteSansJointure: () => sb
                ...
                .or(`hubisoccer_id.eq.${safeId},feed_id.eq.${safeId}`)
                .maybeSingle(),
            liens: [HubJointure.profil('profiles', 'hubisoccer_id', 'hubisoccer_id, full_name, display_name, avatar_url, role_code, bio, city, country, created_at, height, weight, preferred_foot, club, position, nationality, current_status, discipline, current_level, palmares, specialty, current_diploma, school, schedule_accommodation, hard_skills, study_project, interest_sectors, professional_experiences, soft_skills, availability, certified, feed_id, community_avatar, community_cover, last_seen')]
        });
...
async function loadSuggestions() {
    ...
    // LOT B — trouvé en cliquant le bouton « Rafraîchir ».
    //
    // profileData est nul tant que le profil n'est pas chargé —
    // et il le reste si le chargement a échoué. La suite lisait
    // profileData.sport sans garde et levait :
    //     Cannot read properties of null (reading 'sport')
    // Le bouton paraissait mort ; en réalité il plantait.
    if (!profileData) {
        container.innerHTML = '<p style="font-size:0.78rem;color:var(--gray);">' +
            'Le profil n’est pas encore chargé — les suggestions arriveront après.</p>';
        return;
    }
    ...
    // LOT A — même forme fragile (« profiles:hubisoccer_id »),
    // même filet.
    //
    // Les filtres se posent DANS la fabrique, et une seule fois :
    // ma première version les appliquait deux fois et oubliait la
    // branche « sinon, le pays ».
    const faireRequete = (avecJointure) => {
        let r = sb
            .from('supabaseAuthPrive_communities')
            .select(avecJointure
        ...
        if (profileData.sport)        r = r.eq('sport', profileData.sport);
        else if (profileData.country) r = r.eq('country', profileData.country);

        return r;
    };

    const { data: suggestions } = await HubJointure.avecFilet(sb, {
        requete: () => faireRequete(true),
        requeteSansJointure: () => faireRequete(false),
        liens: [HubJointure.profil('profiles', 'hubisoccer_id',
                                   'hubisoccer_id, role_code, certified, avatar_url')]
    });
...
async function loadPosts(reset = false) {
    ...
    try {
        // LOT A — filet sous DEUX jointures à la fois.
        const { data, error } = await HubJointure.avecFilet(sb, {
            requete: () => sb
