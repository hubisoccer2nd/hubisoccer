...
async function loadTrending() {
    try {
        ...
        // LOT A — la zone des tendances restait vide, et on croyait
        // qu'il n'y avait rien de populaire.
        console.error('Erreur tendances:', e);
        const zone = document.getElementById('trendingList') || document.getElementById('trendingContainer');
        const raison = (typeof describeDbError === 'function') ? describeDbError(e) : (e.message || '');
        if (zone) {
            zone.innerHTML = '<p style="font-size:0.8rem;color:var(--danger);padding:10px;">' +
                'Tendances non chargées — ' + escapeHtml(raison) + '</p>';
        }
    }
}
...
async function searchPosts(query) {
    ...
    //
    // Ce cas est le plus délicat du module : le filtre par rôle
    // porte SUR LA TABLE JOINTE (« author.role_code »). Sans la
    // jointure, ce filtre n'existe plus côté base — il est donc
    // rejoué en JavaScript APRÈS le rattachement.
    //
    // Conséquence honnête : dans ce repli, le compte total et la
    // pagination portent sur les publications AVANT filtrage par
    // rôle. Une page peut donc contenir moins de résultats que
    // PAGE_SIZE. Mieux vaut ça qu'une recherche entièrement vide.
    function base(avecJointure) {
        let r = sb.from('supabaseAuthPrive_posts')
            .select(avecJointure
        ...
        if (avecJointure && activeRole !== 'all') r = r.eq('author.role_code', activeRole);

        if (sortBy === 'recent')       r = r.order('created_at', { ascending: false });
        else if (sortBy === 'popular') r = r.order('likes_count', { ascending: false });
        return r;
    }

    const { data, error, count, viaFilet } = await HubJointure.avecFilet(sb, {
        requete: () => base(true),
        requeteSansJointure: () => base(false),
        liens: [HubJointure.profil('author', 'author_hubisoccer_id',
                                   'hubisoccer_id, full_name, display_name, avatar_url, role_code')]
    });
    if (error) throw error;

    let lignes = data || [];
    if (viaFilet && activeRole !== 'all') {
        lignes = lignes.filter(d => d.author && d.author.role_code === activeRole);
    }
    return { data: lignes.map(d => ({ ...d, resultType: 'post' })), count };
}
