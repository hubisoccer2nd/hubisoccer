...
async function loadBlockedUsers() {
    ...
    try {
        ...
        const { data } = await HubJointure.avecFilet(sb, {
            requete: () => sb.from('supabaseAuthPrive_blocked_users')
                .select('blocked_hubisoccer_id, profile:supabaseAuthPrive_profiles!blocked_hubisoccer_id(full_name, display_name, avatar_url)')
                .eq('user_hubisoccer_id', currentProfile.hubisoccer_id),
            requeteSansJointure: () => sb.from('supabaseAuthPrive_blocked_users')
                .select('blocked_hubisoccer_id')
                .eq('user_hubisoccer_id', currentProfile.hubisoccer_id),
            liens: [HubJointure.profil('profile', 'blocked_hubisoccer_id',
