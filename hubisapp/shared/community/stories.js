...
function renderCurrentStory() {
    ...
    // Vérifier la limite de 10 minutes par groupe
    ...
    // Afficher les bons boutons d'options
    ...
    // Nettoyer les anciens médias (garder le loader)
    while (mediaArea.children.length > 1) {
        mediaArea.removeChild(mediaArea.lastChild);
    }

    // Définir la durée (minimum 5 secondes, maximum 3600 = 60 minutes)
...
function closeViewer() {
    ...
    // Retirer le groupe temporaire "ma story" s'il existe
    if (storyGroups[0]?.isOwn) {
        storyGroups.shift();
    }
    // Rafraîchir la liste après visionnage
    loadAllStories();
}
...
async function markStoryViewed(storyId, authorId) {
    ...
    try {
        ...
        // Silencieux, ne pas bloquer l'expérience
