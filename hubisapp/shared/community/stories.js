// ============================================================
//  HUBISOCCER — STORIES.JS (VERSION CORRIGÉE)
//  Visionneuse complète de stories — 15 min support
// ============================================================

'use strict';

// sb, currentUser, currentProfile sont déjà définis dans session.js
// On les utilise directement, sans les redéclarer.
...
function handleStoryFileSelect(file) {
    const maxSize = file.type.startsWith('video/') ? 900 * 1024 * 1024 : 10 * 1024 * 1024; // Vidéo jusqu'à 900 Mo (~15 min)
    if (file.size > maxSize) { toast(`Fichier trop volumineux (max ${file.type.startsWith('video/') ? '900' : '10'} Mo)`, 'warning'); return; }
