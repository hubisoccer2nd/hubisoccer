...
// ═══════════════════════════════════════════════════════════
// 13. (fonction retirée — le tirage crée désormais l'équipe
//     lui-même, il n'y a plus d'équipe existante à sélectionner
...
async function spinWheel() {
    ...
    if (!currentTournamentId) {
        showToast('Veuillez sélectionner un tournoi.', 'warning');
        return;
    }
...
// ═══════════════════════════════════════════════════════════
// 18. FINALISATION DU TIRAGE
// ------------------------------------------------------------
// Le tirage CREE l'equipe -- il ne l'attribue pas a une equipe
// qui existerait deja. Le nom du club tire DEVIENT le nom de
// l'equipe : on ne peut evidemment pas demander de nommer une
// equipe avant de savoir, par le tirage, comment elle s'appellera.
// ═══════════════════════════════════════════════════════════
async function finalizeDraw(repName, sliceAngle, numSlices) {
    ...
    // 1. Cree l'equipe MAINTENANT, avec le club tire comme nom --
    // c'est le tirage qui donne son identite a l'equipe, pas l'inverse.
    const { data: newTeam, error: teamError } = await supabaseClient
        .from(TBL_TEAMS)
        .insert([{
            tournament_id: currentTournamentId,
            name: winningClub.name,
            logo_url: winningClub.logo,
    ...
    if (teamError) {
        hideLoader();
        showToast('Erreur lors de la création de l\'équipe : ' + teamError.message, 'error');
        return;
    }

    // 2. Enregistre le tirage, relie a l'equipe qui vient d'etre creee.
    ...
    if (drawError) {
        // L'equipe est deja creee -- on la retire pour ne pas laisser
        // une equipe fantome sans tirage associe.
        await supabaseClient.from(TBL_TEAMS).delete().eq('id', newTeam.id);
        if (drawError.code === '23505') {
            showToast('Ce club vient d\'être pris. Rafraîchissez et réessayez.', 'warning');
        } else {
            showToast('Erreur lors de l\'enregistrement du tirage : ' + drawError.message, 'error');
        }
        await loadAvailableClubs();
        return;
    }

    showResultModal(repName, winningClub, newTeam.name);
    await loadAvailableClubs();
...
// 19. MODALE RÉSULTAT
// ═══════════════════════════════════════════════════════════
function showResultModal(repName, club, teamName) {
    document.getElementById('resultLogo').src = club.logo;
    document.getElementById('resultText').innerHTML =
        'Votre équipe s\'appelle désormais <strong>' + escapeHtml(teamName) + '</strong> ! ' +
        escapeHtml(repName) + ' la représentera durant toute la compétition.';
