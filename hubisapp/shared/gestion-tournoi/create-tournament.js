...
async function loadSports() {
    const { data } = await supabaseClient
        .from('supabaseAuthPrive_gt_sports')
        .select('id, name')
        .order('name');
...
async function loadTypes() {
    const { data } = await supabaseClient
        .from('supabaseAuthPrive_gt_types')
...
async function handleSubmit(e) {
    ...
    showLoader();
    const { data: nouveau, error } = await supabaseClient
        ...
        .insert([{
            ...
            // ============================================================
            //  PROPRIETAIRE DU TOURNOI — CORRECTION MAJEURE
            // ------------------------------------------------------------
            //  Aucune colonne de proprietaire n'etait enregistree ici.
            //  Consequences :
            //    - my-tournaments.js filtre sur .eq('created_by', ...)
            //      et ne trouvait donc JAMAIS rien : « Mes tournois »
            //      etait structurellement vide ;
            //    - aucune page ne pouvait verifier qui avait le droit
            //      de gerer le tournoi, d'ou l'acces libre au panneau
            //      de gestion avec un simple ?id= dans l'URL.
            //
            //  Une seule colonne de proprietaire existe reellement
            //  dans la table : created_by. Verifie sur le schema.
            // ============================================================
            // La table possede DEJA la colonne created_by (uuid).
            // Elle n'etait simplement jamais renseignee : c'est pour
            // cela que « Mes tournois », qui filtre dessus, restait
            // vide quoi qu'on fasse.
            //
            // Verifie contre le schema reel : organizer_id et user_id
            // n'existent PAS sur cette table. Les ecrire provoquerait
            // une erreur 42703 et le tournoi ne serait pas cree.
    ...
    if (error) {
        ...
        // On envoie l'organisateur vers la GESTION de son tournoi,
        // et non vers l'accueil : c'est la suite naturelle, et cela
        // rend enfin accessible manage-tournament.html, qui n'etait
        // atteignable par aucun lien du depot.
        setTimeout(function() {
            if (nouveau && nouveau.id) {
                window.location.href = 'manage-tournament.html?id=' + encodeURIComponent(nouveau.id);
            } else {
