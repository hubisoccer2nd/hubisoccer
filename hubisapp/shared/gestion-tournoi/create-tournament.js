...
async function loadSports() {
    ...
    // Sans la liste des sports, on ne peut pas créer de tournoi :
    // ce n'est pas un détail, ça doit se voir.
    if (error) { signalerEchec('La liste des sports n\'a pas pu être chargée — ' +
                               'impossible de créer un tournoi tant qu\'elle est vide', error); return; }
...
async function loadTypes() {
    ...
    if (error) { signalerEchec('La liste des types de tournoi n\'a pas pu être chargée', error); return; }
...
async function handleSubmit(e) {
    ...
    // Configuration du format : colonnes existantes (format_type,
    // qualifiers_*, qualification_explainer) + nouvelles colonnes
    // (format_family, format_config, bareme, departage, zones).
    // Renvoie null si aucun format n'a ete choisi -- le tournoi est
    // alors cree sans format, et il reste modifiable dans
    // « Gerer un tournoi ».
    const configurationFormat = GTFormats.lire();
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // Editeur de format de competition (gt-formats.js).
    // Sans lui, format_type restait NULL et rankings.js retombait
    // toujours sur « league ».
    GTFormats.monterEditeur({ conteneur: 'gtFormatEditeur' });
