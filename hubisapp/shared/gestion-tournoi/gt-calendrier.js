...
window.GTCalendrier = (function () {
    ...
    function genererDepuisFormat(codeFormat, famille, valeurs, equipes, options) {
        ...
        if (famille === 'mixte') {
            ...
            // CHANTIER 19 — cette branche rendait  avertissement: null.
            //
            // Elle ne cree QUE les poules, et c'est normal : le
            // tableau final depend du classement, qui n'existe pas
            // encore. Mais elle ne le DISAIT pas. L'organisateur
            // croyait son calendrier complet, cliquait plus tard sur
            // « Faire avancer le tableau », et recevait « ces
            // rencontres ne font pas partie d'un tableau » sans
            // comprendre pourquoi.
            //
            // Le silence etait le vrai bug. Le tableau, lui, se cree
            // maintenant depuis l'onglet Matchs, avec « Creer la
            // phase finale ».
            var qual = valeurs.qualifiesParGroupe || 0;
            var repeches = valeurs.meilleursTroisiemes || 0;
            var attendus = qual * nbGroupes + repeches;

            return {
                rencontres: rencontres,
                groupes: repartition,
                avertissement: 'Les ' + rencontres.length + ' rencontres de poule sont prêtes. ' +
                    'La phase finale n\'est PAS encore créée : elle dépend du classement, ' +
                    'qui n\'existera qu\'une fois les poules jouées. ' +
                    (attendus > 1
                        ? 'Quand elles seront terminées, le bouton « Créer la phase finale » ' +
                          'dessinera le tableau des ' + attendus + ' qualifiés.'
                        : 'Quand elles seront terminées, utilise le bouton « Créer la phase finale ».')
            };
        }
