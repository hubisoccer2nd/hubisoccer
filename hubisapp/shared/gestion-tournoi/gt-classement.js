...
window.GTClassement = (function () {
    ...
    // CHANTIER 11 — le troisieme parametre, le nom du groupe.
    // ------------------------------------------------------
    // Sans lui, la ligne enregistree ne disait pas de quelle
    // poule elle venait. La page de l'organisateur ne s'en
    // apercevait pas (elle recalcule tout en memoire), mais le
    // visiteur qui LIT gt_standings ne pouvait afficher qu'une
    // liste plate. Il est facultatif : un championnat sans
    // poules l'omet et rien ne change pour lui.
    function pourLaBase(lignes, idTournoi, nomGroupe) {
        return lignes.map(function (l) {
            return {
                tournament_id: idTournoi,
                group_name: nomGroupe || l.group_name || null,
                team_id: l.team_id,
                played: l.played,
                wins: l.wins,
                draws: l.draws,
