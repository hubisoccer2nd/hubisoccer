/* ============================================================
   ...
   Un parrain, un formateur ou un sportif peut être désigné
...
window.GTOfficiels = (function () {
    ...
    // ═══════════════════════════════════════════════════════
    // LE MOT DE LA DISCIPLINE (chantier 08)
    // -------------------------------------------------------
    // Les libelles de ce module peuvent contenir {Sportif},
    // {pluriel}, {collectif}… Ils sont resolus au moment du
    // rendu, avec la discipline du tournoi passee par la page
    // appelante : monterFormulaire({ sport: 'Basket-ball' }).
    //
    // Sans gt-lexique.js charge, ou sans sport connu, le gabarit
    // retombe sur « Sportif » — jamais sur « joueur ».
    // ═══════════════════════════════════════════════════════
    var sportCourant = '';

    function definirSport(nom) { sportCourant = nom || ''; }

    function motDuSport(texte) {
        if (texte == null) return texte;
        if (!window.GTLexique) {
            // Repli sans le lexique : on respecte la casse
            // demandee par le gabarit. {Sportif} donne « Sportif »,
            // {sportif} donne « sportif » — sans quoi une majuscule
            // se planterait au milieu d'une phrase.
            return String(texte).replace(/\{([A-Za-z]+)\}/g, function (tout, cle) {
                return cle.charAt(0) === cle.charAt(0).toUpperCase() ? 'Sportif' : 'sportif';
            });
        }
        return window.GTLexique.remplir(texte, sportCourant);
    }

    // « le sportif », « l'athlète », « le basketteur ». L'article
    // defini demande une elision que remplir() ne fait pas : c'est
    // le travail de GTLexique.defini().
    function leSportif() {
        if (!window.GTLexique) return 'le sportif';
        return window.GTLexique.defini(sportCourant);
    }

    // Un libelle : d'abord le mot juste, ensuite l'echappement.
    // Jamais l'inverse — echapper d'abord transformerait les
    // accolades et le gabarit ne serait plus reconnu.
    function libelleEchappe(texte) { return echapper(motDuSport(texte)); }
    ...
    function rendreChamp(champ, valeur, contexte) {
        ...
        if (champ.type === 'bool') {
            return '<div class="gto-champ gto-champ-bool">' +
                   ...
                   '<label for="' + id + '">' + libelleEchappe(champ.libelle) + '</label></div>';
        }

        if (champ.type === 'longtexte') {
            return '<div class="gto-champ gto-champ-large">' +
                   '<label for="' + id + '">' + libelleEchappe(champ.libelle) + '</label>' +
        ...
        if (champ.type === 'choix') {
            ...
            return '<div class="gto-champ"><label for="' + id + '">' + libelleEchappe(champ.libelle) + '</label>' +
    ...
    function rendreListe(champ, valeur, contexte) {
        ...
        return '<div class="gto-liste" data-liste="' + champ.cle + '">' +
               '<div class="gto-liste-tete">' +
                   '<span>' + libelleEchappe(champ.libelle) + '</span>' +
    ...
    function rendreLigneListe(champ, ligne, contexte) {
        ...
        var cellules = champ.colonnes.map(function (col) {
            ...
            if (col.type === 'equipe') {
                return '<div class="' + classe + '"><label>' + libelleEchappe(col.libelle) + '</label>' +
            ...
            if (col.type === 'bool') {
                return '<div class="' + classe + ' gto-cellule-bool"><label>' + libelleEchappe(col.libelle) + '</label>' +
    ...
    function monterFormulaire(options) {
        ...
        if (options.sport !== undefined) definirSport(options.sport);
        ...
        hote.innerHTML = blocs.map(function (nomBloc) {
            ...
            return '<section class="gto-bloc" data-bloc="' + nomBloc + '">' +
                   '<h3 class="gto-bloc-titre">' + libelleEchappe(bloc.titre) + '</h3>' +
                   (bloc.aide ? '<p class="gto-bloc-aide">' + libelleEchappe(bloc.aide) + '</p>' : '') +
    ...
    function verifier(acteurCode, contenu) {
        ...
        (contenu.cartonsJaunes || []).forEach(function (c, index) {
            if (!c.joueur) fautes.push('Carton jaune n°' + (index + 1) + ' : ' + leSportif() + ' n\'est pas renseigné.');
        });
        rouges.forEach(function (c, index) {
            if (!c.joueur) fautes.push('Carton rouge n°' + (index + 1) + ' : ' + leSportif() + ' n\'est pas renseigné.');
        });
    ...
    return {
        ...
        definirSport: definirSport,
        motDuSport: motDuSport,
        leSportif: leSportif,
