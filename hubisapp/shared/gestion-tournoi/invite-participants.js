/* ============================================================
   ...
   pratiquants de la plateforme (role_code='FOOT'), en excluant
...
// Chantier 08 — la discipline du tournoi, resolue au chargement.
// Tant qu'elle est vide le lexique dit « sportif », jamais
// « joueur » et jamais « footballeur » : cette page sert aussi
// bien un tournoi de basket qu'un concours de chant.
let nomSportTournoi = '';
...
async function loadTournamentInfo() {
    ...
    if (!currentTournamentId) {
        GTPicker.monter({
            ...
            portee: 'mesTournois',
            icone: 'fa-user-plus',
            titre: 'À quel tournoi voulez-vous inviter des participants ?',
    ...
    const { data } = await supabaseClient
        ...
        .select('name, start_date, end_date, created_by, sport_id')
        .eq('id', currentTournamentId)
        .maybeSingle();

    // Verrou de propriété : on n'invite que sur SON propre tournoi.
    // Même modèle que manage-tournament.js.
    if (!data) {
        currentTournamentId = null;
        afficherRefus('Ce tournoi est introuvable.',
                      'Il a peut-être été supprimé, ou le lien est incorrect.');
        return;
    }
    if (data.created_by !== currentUser.id) {
        // On efface le contexte pour que les chargements suivants
        // (équipes, participants) ne partent même pas en requête.
        currentTournamentId = null;
        afficherRefus('Ce tournoi ne vous appartient pas.',
                      'Seul l\'organisateur qui a créé un tournoi peut y inviter des participants.');
        return;
    }

    // Chantier 08 — le nom de la discipline. Requete separee sur
    // gt_sports plutot qu'une jointure : c'est la regle suivie
    // partout ici depuis l'incident sur manage-tournament, ne
    // jamais dependre d'une relation non verifiee.
    if (data.sport_id) {
...
// Affiche un panneau d'accès refusé à la place du contenu de la page.
function afficherRefus(titre, explication) {
    const hote = document.getElementById('gtPicker');
    if (!hote) return;
    const parent = hote.parentNode;
    Array.prototype.forEach.call(parent.children, function (enfant) {
        if (enfant === hote) return;
        if (enfant.id === 'backBtn' || (enfant.classList && enfant.classList.contains('btn-back'))) return;
        enfant.style.display = 'none';
    });
    hote.className = 'gt-refus';
    hote.innerHTML =
        '<i class="fas fa-lock"></i>' +
        '<h2>' + titre + '</h2>' +
        '<p>' + explication + '</p>' +
        '<a href="my-tournaments.html"><i class="fas fa-list"></i> Voir mes tournois</a>';
...
// ═══════════════════════════════════════════════════════════
// 13. PRATIQUANTS DE LA PLATEFORME (correction de logique)
// ------------------------------------------------------------
// Cherche parmi TOUS les pratiquants (role_code='FOOT'), pas
...
async function loadPlayers(searchTerm) {
    ...
    if (error) { showToast(mot('Erreur recherche {pluriel}'), 'error'); return; }

    const results = (data || []).filter(function(p) { return !excludedIds.has(p.auth_uuid); });

    const container = document.getElementById('playersList');
    if (!results.length) { container.innerHTML = '<p class="empty-hint">' + escapeHtml(mot('Aucun {sportif} trouvé (ou déjà candidat à ce tournoi).')) + '</p>'; return; }
    ...
    container.innerHTML = results.map(function(p) {
        return '<div class="item-card">' +
               ...
               '<div class="item-info"><div class="item-name">' + escapeHtml(p.full_name || 'Inconnu') + '</div><div class="item-detail">' + escapeHtml(mot('{Sportif}')) + '</div></div>' +
...
// ═══════════════════════════════════════════════════════════
// 16. INVITER UN PRATIQUANT / UNE ÉQUIPE
// ═══════════════════════════════════════════════════════════
...
async function inviteTeam(teamId) {
    ...
    if (!members || members.length === 0) {
        showToast(mot('Cette {collectif} n\'a pas de {pluriel}.'), 'warning');
        return;
    }
    showLoader();
