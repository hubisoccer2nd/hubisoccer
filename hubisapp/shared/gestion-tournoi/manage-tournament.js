...
function renderTeams() {
    ...
    container.innerHTML = allTeams.map(function(team) {
        ...
        return '<div class="team-card">' + logo +
               ...
               '<button class="btn-roster" onclick="openRoster(\'' + team.id + '\', \'' + escapeHtml(team.name).replace(/'/g, "\\'") + '\')"><i class="fas fa-users"></i> Effectif</button>' +
...
// ═══════════════════════════════════════════════════════════
// EFFECTIF D'UNE ÉQUIPE (nouvelle fonctionnalité)
// ------------------------------------------------------------
// La table supabaseAuthPrive_gt_team_players existe depuis la
// migration initiale mais n'etait utilisee nulle part sur cette
// page -- aucun moyen de voir ou constituer un effectif.
// ═══════════════════════════════════════════════════════════
let currentRosterTeamId = null;

async function openRoster(teamId, teamName) {
    currentRosterTeamId = teamId;
    document.getElementById('rosterTeamId').value = teamId;
...
async function loadRoster(teamId) {
    ...
    if (error) {
        console.warn('Table ' + TBL_TEAM_PLAYERS + ' absente ou erreur :', error.message);
        container.innerHTML = '<p class="empty-hint">Effectif indisponible pour l\'instant.</p>';
        return;
    }
    renderRoster(data || []);
}
...
function renderRoster(players) {
    const container = document.getElementById('rosterList');
    if (!players.length) { container.innerHTML = '<p class="empty-hint">Aucun joueur ajouté.</p>'; return; }
    container.innerHTML = players.map(function(p) {
        return '<div class="roster-item">' +
               (p.jersey_number ? '<span class="roster-number tabular">' + escapeHtml(String(p.jersey_number)) + '</span>' : '<span class="roster-number tabular">—</span>') +
               '<span class="roster-name">' + escapeHtml(p.player_name) + '</span>' +
               '<button class="btn-delete-mini" onclick="deletePlayer(\'' + p.id + '\')"><i class="fas fa-times"></i></button>' +
               '</div>';
    }).join('');
}
...
async function addPlayer(e) {
    ...
    if (!name) { showToast('Le nom du joueur est requis.', 'warning'); return; }

    const { error } = await supabaseClient
        .from(TBL_TEAM_PLAYERS)
        .insert([{ team_id: currentRosterTeamId, player_name: name, jersey_number: number ? parseInt(number, 10) : null }]);

    if (error) {
        showToast('Erreur : ' + error.message, 'error');
    } else {
        document.getElementById('addPlayerForm').reset();
        await loadRoster(currentRosterTeamId);
    }
}
...
async function deletePlayer(playerId) {
    ...
    else { await loadRoster(currentRosterTeamId); }
}
