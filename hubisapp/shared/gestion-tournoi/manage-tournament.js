...
// ═══════════════════════════════════════════════════════════
// ============================================================
//  CLIENT SUPABASE ET SESSION : voir session-gt.js
// ------------------------------------------------------------
//  Les declarations qui se trouvaient ici (SUPABASE_URL,
//  SUPABASE_ANON_KEY, supabaseClient, currentUser, userProfile)
//  etaient recopiees a l'identique dans les 20 fichiers du
//  module. Elles vivent desormais dans session-gt.js, charge
//  avant ce script par la page.
//
//  Elles y sont declarees avec « var » et non « const » : deux
//  scripts ne peuvent pas declarer le meme nom avec const sans
//  provoquer une SyntaxError qui tue toute la page.
// ============================================================
// ═══════════════════════════════════════════════════════════
// 2. ÉTAT GLOBAL
// ═══════════════════════════════════════════════════════════
...
async function loadTournament() {
    // getTournamentId() de session-gt.js accepte ?id=, ?tournament_id=
    // et ?tournoi= : les anciens liens continuent de fonctionner.
    currentTournamentId = (typeof getTournamentId === 'function')
        ? getTournamentId()
    ...
    // ============================================================
    //  CONTROLE D'ACCES — CORRECTION MAJEURE
    // ------------------------------------------------------------
    //  Cette page chargeait le tournoi designe par ?id= SANS AUCUNE
    //  verification. N'importe quel utilisateur connecte pouvait
    //  donc ouvrir le panneau de gestion complet du tournoi d'un
    //  autre organisateur : inscriptions, equipes, matchs, rapports
    //  et dotations. Les RLS etant desactives en developpement, la
    //  base ne s'y opposait pas non plus.
    //
    //  requireTournamentOwner() (session-gt.js) verifie la propriete
    //  et renvoie l'intrus vers la fiche publique du tournoi avec un
    //  message clair, au lieu de lui ouvrir l'administration.
    // ============================================================
    const data = await requireTournamentOwner(currentTournamentId);
    hideLoader();

    if (!data) return;   // message et redirection deja pris en charge

    currentTournament = data;

    // Le menu lateral affiche le bloc « Organisation » maintenant
    // que la qualite d'organisateur est etablie.
    if (typeof refreshNavOwnerGT === 'function') {
        refreshNavOwnerGT('manage', currentTournamentId, true);
    }
...
async function loadTeams() {
    const { data, error } = await supabaseClient
        .from('supabaseAuthPrive_gt_teams')
        .select('*')
        .eq('tournament_id', currentTournamentId)
        .order('name');
...
function renderTeams() {
    ...
    allTeams.forEach(function(team) {
        // team-details.html n'etait atteignable par aucun lien :
        // la carte d'equipe y mene desormais.
        // Le logo passe par escapeAttr : une URL contenant un
        // guillemet cassait l'attribut src et le reste de la carte.
        const tid = team.id;
        html += '<div class="team-card">' +
                (team.logo_url
                    ? '<img src="' + escapeAttr(team.logo_url) + '" alt="" class="team-logo">'
                    : '<div class="team-logo-placeholder"><i class="fas fa-users"></i></div>') +
                '<a class="team-name" href="team-details.html?id=' + encodeURIComponent(tid) + '">' +
                    escapeHtml(team.name) + '</a>' +
                '<button class="btn-delete" title="Retirer" onclick="deleteTeam(\'' + escapeAttr(tid) + '\')">' +
...
async function loadMatches() {
    const { data, error } = await supabaseClient
        .from('supabaseAuthPrive_gt_matches')
        .select('*, team_a:supabaseAuthPrive_gt_teams!team_a_id(name), team_b:supabaseAuthPrive_gt_teams!team_b_id(name)')
        .eq('tournament_id', currentTournamentId)
...
function renderMatches(matches) {
    ...
    matches.forEach(function(m) {
        ...
        // ============================================================
        //  REBRANCHEMENT DES PAGES DU MATCH
        // ------------------------------------------------------------
        //  match-details.html, match-report.html et
        //  match-report-export.html n'etaient atteignables par AUCUN
        //  lien du depot. Elles sont desormais accessibles depuis
        //  chaque ligne de match, avec le bon parametre d'URL.
        //
        //  Les identifiants passent par escapeAttr : ils sont
        //  concatenes dans des attributs onclick, ou une apostrophe
        //  cassait le HTML de toute la liste.
        // ============================================================
        const mid = escapeAttr(m.id);
        html += '<div class="match-item">' +
                '<div class="match-date">' + escapeHtml(date) + '</div>' +
                '<div class="match-teams">' + escapeHtml(m.team_a?.name || '?') + ' vs ' + escapeHtml(m.team_b?.name || '?') + '</div>' +
                '<div class="match-score">' + (m.score_a || 0) + ' - ' + (m.score_b || 0) + '</div>' +
                '<div class="match-actions">' +
                    '<a class="btn-icon" title="Fiche du match" href="match-details.html?id=' + encodeURIComponent(m.id) + '">' +
                        '<i class="fas fa-circle-info"></i></a>' +
                    '<a class="btn-icon" title="Feuille de match" href="match-report.html?match_id=' + encodeURIComponent(m.id) + '">' +
                        '<i class="fas fa-file-pen"></i></a>' +
                    '<a class="btn-icon" title="Exporter la feuille" href="match-report-export.html?match_id=' + encodeURIComponent(m.id) + '">' +
                        '<i class="fas fa-file-export"></i></a>' +
                    '<button class="btn-delete" title="Supprimer" onclick="deleteMatch(\'' + mid + '\')">' +
                        '<i class="fas fa-trash"></i></button>' +
                '</div>' +
                '</div>';
    });
...
async function loadReports() {
    ...
    const { data, error } = await supabaseClient
        .from('supabaseAuthPrive_gt_match_reports')
        .select('*')
...
async function loadPrizes() {
    const { data, error } = await supabaseClient
        .from('supabaseAuthPrive_gt_prizes')
        .select('*')
        .eq('tournament_id', currentTournamentId)
        .order('created_at', { ascending: false });
    if (error) {
...
async function saveTournamentChanges(e) {
    ...
    // ============================================================
    //  FORMAT ET SPORT — CORRECTION
    // ------------------------------------------------------------
    //  Cette fonction mettait a jour 9 champs mais ni type_id ni
    //  sport_id. Le format choisi a la creation etait donc
    //  DEFINITIVEMENT fige : selectionner un autre format n'avait
    //  aucun effet sur le tournoi deja cree.
    //
    //  Les deux listes ne sont presentes que si le HTML les
    //  fournit : on ne casse rien si elles manquent encore.
    // ============================================================
    const champType = document.getElementById('editType');
    if (champType && champType.value) {
        updates.type_id = champType.value;
    }

    const champSport = document.getElementById('editSport');
    if (champSport && champSport.value) {
        updates.sport_id = champSport.value;
    }
    showLoader();
    const { error } = await supabaseClient
    ...
    if (error) {
        ...
        const { data } = await supabaseClient
            .from('supabaseAuthPrive_gt_tournaments')
            .select('*, sport:supabaseAuthPrive_gt_sports(name), type:supabaseAuthPrive_gt_types(name, label)')
            .eq('id', currentTournamentId)
            .single();
        if (data) {
            currentTournament = data;
