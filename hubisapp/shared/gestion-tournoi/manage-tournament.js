...
async function loadRegistrations() {
    ...
    if (error) {
        signalerEchec('Les inscriptions n\'ont pas pu être chargées', error);
...
async function loadTeams() {
    const { data, error } = await supabaseClient
        ...
        .eq('tournament_id', currentTournamentId)
        .order('name');
    // Une liste d'équipes vide sans explication, c'est
    // l'organisateur qui croit que personne ne s'est inscrit.
    if (error) { signalerEchec('Les équipes du tournoi n\'ont pas pu être chargées', error); return; }
    allTeams = data || [];
    renderTeams();
}
...
async function verifierResultatsEnregistres() {
    ...
    if (error) {
        signalerEchec('Impossible de vérifier quels matchs ont déjà un résultat', error,
                      { ton: 'warning' });
        matchsAvecResultat = [];
...
async function recalculerLesStatistiques() {
    ...
    for (let i = 0; i < rencontres.length; i++) {
        ...
        for (let j = 0; j < retenues.length; j++) {
            ...
            if (erreur) {
                signalerEchec('Feuille de match non écrite (rencontre ' + rencontre.id + ')', erreur);
                erreurs++;
            } else {
                ecrites++;
...
async function chargerLesStatistiquesDuTournoi() {
    ...
    if (error) {
        signalerEchec('Les cumuls de statistiques n\'ont pas pu être lus', error, { ton: 'warning' });
        return;
    }
...
async function chargerLeCatalogue() {
    ...
    if (error) {
        ...
        signalerEchec('Les moyens de paiement autorisés n\'ont pas pu être lus', error,
                      { ton: 'warning' });
        cataloguePaiement = [];
        return;
    }
...
async function chargerLAccord() {
    ...
    if (error) {
        signalerEchec('Ton accord d\'organisateur n\'a pas pu être lu', error, { ton: 'warning' });
    }
...
async function loadMatches() {
    const { data, error } = await supabaseClient
        ...
        .order('match_date', { ascending: true });
    if (error) {
        signalerEchec('Les matchs n\'ont pas pu être chargés', error);
...
async function loadReports() {
    ...
    if (error) { signalerEchec('Les rapports de match n\'ont pas pu être chargés', error); return; }
    renderReports(data || []);
}
...
async function loadPrizes() {
    ...
    if (error) {
        ...
        signalerEchec('Les primes n\'ont pas pu être chargées', error);
