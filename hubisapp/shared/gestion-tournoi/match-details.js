...
// Chantier 08 — la discipline du tournoi de ce match. Resolue au
// chargement du match, donc AVANT le fil d'evenements, les
// compositions et les statistiques : les trois nomment le
// pratiquant, ils doivent tous employer le meme mot.
let nomSportTournoi = '';
...
async function loadMatchDetails(matchId) {
    ...
    if (match.tournament_id) {
        const { data: tournament } = await supabaseClient.from(TBL_TOURNAMENTS).select('name, stream_url, sport_id').eq('id', match.tournament_id).maybeSingle();
        if (tournament) { tournamentName = tournament.name; streamUrl = tournament.stream_url; }
        // Chantier 08 — le nom de la discipline, une seule fois.
        if (tournament && tournament.sport_id) {
            const { data: sp } = await supabaseClient
                .from(TBL_TOURNAMENTS_SPORTS).select('name').eq('id', tournament.sport_id).maybeSingle();
...
async function loadMatchEvents(idMatch) {
    ...
    if (error) {
        signalerEchec('Les événements du match n\'ont pas pu être lus', error, { ton: 'warning' });
...
async function loadMatchReports(matchId) {
    ...
    if (error) {
        signalerEchec('Les rapports des officiels n\'ont pas pu être lus', error, { ton: 'warning' });
...
async function monterLesCompositions(match) {
    ...
    if (etat) {
        ...
        etat.innerHTML = '<i class="fas fa-circle-check"></i> ' + total + ' ' + escapeHtml(mot('{sportif}(s)')) + ' sur les feuilles' +
...
function placeCompositionHtml(ligne, x, y, libelleParDefaut) {
    const nom = ligne.member_name || mot('{Sportif}');
    const etats = [];
...
async function chargerStatistiquesDuMatch(idMatch) {
    ...
    if (error) {
        signalerEchec('Les statistiques du match n\'ont pas pu être lues', error, { ton: 'warning' });
        statsDuMatch = [];
        return;
    }
    statsDuMatch = data || [];
...
function rendreTableauStatistiques() {
    ...
    if (!statsDuMatch.length) {
        conteneur.innerHTML = '<div class="gt-stats-vide">' +
            ...
            (peutGererLesStats
                ? mot('Le bouton <em>Calculer depuis les rapports</em> lit les buts, passes décisives, cartons et remplacements déjà saisis dans les rapports de match et en tire une feuille par {sportif}. Les relevés d\'observation — tirs, duels, distance — se saisissent ensuite à la main.')
...
async function recalculerLesStatistiquesDuMatch() {
    ...
    if (!retenues.length) {
        hideLoader();
        showToast(mot('Les événements de ce match ne désignent aucun {sportif} identifié.'), 'warning');
        return;
    }
    ...
    // CHANTIER 20 — on retient le PREMIER motif d'échec.
    // « voir la console » ne sert à personne au bord d'un terrain.
    let premierMotifStat = null;
    ...
    for (let i = 0; i < retenues.length; i++) {
        ...
        if (erreur) {
            ...
            if (!premierMotifStat) premierMotifStat = erreur;
            erreurs++;
        } else {
            ecrites++;
        }
    ...
    const motifStat = premierMotifStat
        ? ((premierMotifStat.message || 'erreur inconnue') +
           (premierMotifStat.code ? ' (' + premierMotifStat.code + ')' : ''))
        : 'motif inconnu';
    ...
    if (etat) {
        etat.innerHTML = '<i class="fas fa-circle-check"></i> ' + ecrites + ' ' + escapeHtml(mot('feuille(s) de {sportif}')) + ' écrites à partir de ' +
                         evenements.length + ' événement(s), le ' +
                         new Date().toLocaleString('fr-FR', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' }) + '.' +
                         (erreurs ? ' ' + erreurs + ' en échec — ' + escapeHtml(motifStat) + '.' : '');
    }

    // CHANTIER 20 — plus de message VERT sur un échec.
    //
    // La version d'avant annonçait « X statistique(s)
    // recalculée(s) » en vert même quand la moitié n'était pas
    // passée, et renvoyait à la console pour le motif.
    if (erreurs) {
        showToast(erreurs + ' statistique(s) sur ' + (erreurs + ecrites) +
                  ' N\'ONT PAS ÉTÉ ENREGISTRÉES — ' + motifStat + '. ' +
                  (premierMotifStat && (premierMotifStat.code === '42703' || premierMotifStat.code === '42P01')
                     ? 'Une colonne manque dans la base : ouvre gt-diagnostic.html, lance l\'analyse ' +
                       'et exécute le SQL qu\'elle propose.'
                     : 'Recommence ; si ça persiste, ouvre gt-diagnostic.html.'), 'error');
    } else {
...
function ouvrirLaSaisie(idSportif) {
    ...
    if (!sportifs.length) {
        ...
        panneau.innerHTML = '<div class="gt-stats-vide">' + escapeHtml(mot('Aucun {sportif} identifié sur ce match.')) + ' ' +
    ...
    html += '<div class="gt-saisie-champ" style="max-width:340px;margin-bottom:16px;">' +
        '<label for="statsSportifSelect">' + escapeHtml(mot('{Sportif}')) + '</label>' +
