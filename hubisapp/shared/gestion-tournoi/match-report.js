...
async function chargerMesDesignations() {
    ...
    if (error) {
        signalerEchec('Tes désignations d\'officiel n\'ont pas pu être lues', error,
                      { ton: 'warning' });
        return [];
    }
...
async function submitReport() {
    ...
    if (evenements.length) {
        ...
        if (erreurEvenements) {
            ...
            // Une écriture perdue en silence, c'est le rapport de
            // l'arbitre qui disparaît. Ça ne peut pas rester muet.
            signalerEchec('LES ÉVÉNEMENTS DU MATCH N\'ONT PAS ÉTÉ ENREGISTRÉS', erreurEvenements);
        }
    }
