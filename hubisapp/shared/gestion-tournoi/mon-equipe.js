...
// glisser.
// ═══════════════════════════════════════════════════════════
const DUREE_APPUI_LONG = 500;   // millisecondes
const SEUIL_DEPLACEMENT = 8;    // pixels avant de considérer un glissement
const TOLERANCE_IMMOBILE = 4;   // au-delà, le doigt a bougé : ce n'est plus un appui
...
function demarrerLeGeste(e) {
    ...
    // L'appui long ouvre la fiche, avec une vibration courte pour
    // dire que le geste a été reçu.
    gesteEnCours.minuteur = setTimeout(function() {
        if (!gesteEnCours || gesteEnCours.deplace) return;
        gesteEnCours.appuiLongFait = true;
        vibrer(18);
        ouvrirLaFicheDuMembre(idMembre);
    }, DUREE_APPUI_LONG);
...
function suivreLeGeste(e) {
    ...
    // Un appui long, c'est un appui IMMOBILE. Dès que le doigt
    // bouge, le minuteur est annulé — sinon un glissement un peu
    // lent voyait la fiche s'ouvrir en plein déplacement, et le
    // sportif restait collé à sa place.
    if (!gesteEnCours.deplace &&
        (Math.abs(dx) > TOLERANCE_IMMOBILE || Math.abs(dy) > TOLERANCE_IMMOBILE)) {
        clearTimeout(gesteEnCours.minuteur);
        gesteEnCours.minuteur = null;
    }

    if (!gesteEnCours.deplace) {
        if (Math.abs(dx) < SEUIL_DEPLACEMENT && Math.abs(dy) < SEUIL_DEPLACEMENT) return;
        if (gesteEnCours.appuiLongFait) return;      // la fiche est déjà ouverte
    ...
    // Le sportif suit le doigt sans quitter son conteneur.
...
function finirLeGeste(e) {
    ...
    geste.element.classList.remove('deplace');
    ...
    // Un appui bref, sans déplacement : la fiche s'ouvre. C'est
    // le geste du PC non tactile.
    if (!geste.deplace) {
        if (!geste.appuiLongFait) ouvrirLaFicheDuMembre(geste.idMembre);
        return;
    }

    deposer(geste, e.clientX, e.clientY);
}

function annulerLeGeste() {
    if (!gesteEnCours) return;
    clearTimeout(gesteEnCours.minuteur);
    gesteEnCours.element.classList.remove('deplace');
    gesteEnCours.element.style.transform = '';
    document.getElementById('terrain')?.classList.remove('depot-actif');
    document.getElementById('bancListe')?.classList.remove('depot-actif');
    nettoyerLeGeste();
...
async function ouvrirLaFicheDuMembre(idMembre) {
    ...
    const identite = [
        ...
        { cle: 'Catégorie', valeur: (MEMBER_CATEGORIES[membre.member_category] || {}).label || 'Footballeur' },
    ...
    let html = '<div class="gt-fiche-tete">' +
        ...
        '<div class="gt-fiche-sous">' + escapeHtml(membre.position_detail || sportCourant.nomSportif) +
