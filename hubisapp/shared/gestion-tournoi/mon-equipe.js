...
async function loadUserTeams() {
    // Récupérer les équipes où l'utilisateur est membre (table team_players) ou créateur (via un champ creator_id dans supabaseAuthPrive_gt_teams)
    // Pour l'instant, nous utilisons une colonne creator_id que nous allons ajouter à la table supabaseAuthPrive_gt_teams
    const { data, error } = await supabaseClient
...
async function loadTeamPlayers(teamId) {
    const { data: playersData, error } = await supabaseClient
        .from('supabaseAuthPrive_gt_team_players')
