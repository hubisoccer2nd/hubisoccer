...
async function chargerLesMatchsDeLEquipe() {
    ...
    if (error) {
        signalerEchec('Les rencontres de ton équipe n\'ont pas pu être lues', error, { ton: 'warning' });
    } else {
...
async function loadRoster() {
    ...
    if (error) {
        signalerEchec('L\'effectif n\'a pas pu être chargé', error);
...
async function chargerLaDiscipline() {
    ...
    if (rMatchs.error || rEvenements.error) {
        // Sans ce calcul, un suspendu peut être aligné : le match
        // se perd sur tapis vert. C'est tout sauf un détail.
        signalerEchec('LES SUSPENSIONS N\'ONT PAS PU ÊTRE CALCULÉES — ' +
                      'vérifie toi-même avant d\'aligner', (rMatchs.error || rEvenements.error));
        return;
    }
...
async function chargerLaComposition() {
    ...
    if (matchChoisi) {
        ...
        if (error) {
            signalerEchec('La composition enregistrée n\'a pas pu être relue', error);
        } else {
