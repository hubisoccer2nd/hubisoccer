...
async function loadMyTournaments() {
    ...
    const { data, error } = await supabaseClient
        ...
        // CHANTIER 17 — select('*') plutot qu'une liste de
        // colonnes. is_public n'existe peut-etre pas encore (le
        // script SQL du chantier 17 l'ajoute), et une colonne
        // nommee mais absente fait echouer TOUTE la requete
        // (42703) : la liste des tournois resterait vide.
        .select('*')
    ...
    if (allMyTournaments.length) {
        ...
        if (errParticipants) {
            signalerEchec('Le nombre de participants n\'a pas pu être compté — ' +
                          'les cartes afficheront 0', errParticipants, { ton: 'warning' });
        } else {
            const compteurs = {};
...
function renderTournaments(tournaments) {
    ...
    container.innerHTML = tournaments.map(function(t) {
        ...
        const estPublic = t.status === 'published' && t.is_public !== false;
        ...
        return '<div class="my-tournament-card">' +
               ...
               // CHANTIER 17 — le lien qu'on envoie aux parents,
               // aux sponsors, aux spectateurs. Il n'apparait que
               // sur un tournoi REELLEMENT public : un brouillon
               // afficherait « pas encore public » au visiteur.
               // is_public absent = public, la colonne peut ne pas
               // exister encore.
               (estPublic
                   ? '<a href="tournoi-public.html?id=' + t.id + '" target="_blank" rel="noopener" ' +
                     'class="btn-action"><i class="fas fa-share-nodes"></i> Vue publique</a>'
                   : '') +
