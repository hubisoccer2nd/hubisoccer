...
// ═══════════════════════════════════════════════════════════
// 10. DÉTERMINATION DU SPORTIF CONSULTÉ (?id= ou soi-même)
// ═══════════════════════════════════════════════════════════
...
async function loadViewedPlayerProfile() {
    ...
    if (error || !data) {
        showToast('Sportif introuvable.', 'error');
        return;
    }
...
async function updatePlayerHero() {
    ...
    // Chantier 08 — cette page agrege les statistiques d'une
    // personne SUR PLUSIEURS TOURNOIS, donc potentiellement sur
    // plusieurs disciplines : aucune ne peut etre designee comme
    // « la » discipline de la page. Le terme neutre est ici le
    // seul juste. Le titre et le libelle du menu, eux, restent
    // « Statistiques footballeur » comme tu l'as fixe au point 30.
    document.getElementById('playerName').textContent = viewedPlayerProfile.full_name || 'Sportif';
