/* ============================================================
   ...
   - Buteurs/Passeurs/Cartons etaient de simples messages
     ...
     agrege par pratiquant sur le tournoi selectionne.
...
// Chantier 08 — la discipline suit le tournoi choisi. On retient
// sport_id par tournoi au chargement de la liste : changer de
// tournoi change le vocabulaire sans requete supplementaire.
let sportParTournoi = {};
let nomSportTournoi = '';
...
// ═══════════════════════════════════════════════════════════
// Chantier 05 : le classement des sportifs ne lisait que quatre
// colonnes et triait a la main, sans regle d'egalite. Il passe
// desormais par le meme moteur que la fiche du sportif :
// GTStats.agregerTournoi() pour le cumul, GTStats.classer() pour
// le rang — a nombre egal, celui qui a joue moins de minutes
// passe devant, et les ex aequo portent le meme rang.
async function loadPlayerRankings() {
    ...
    if (error) {
        renderEmptyPlayerRankings('Statistiques indisponibles : ' + error.message);
        return;
    }

    if (!stats || !stats.length) {
        renderEmptyPlayerRankings(
            escapeHtml(mot('Aucune statistique de {sportif} pour ce tournoi.')) + ' Elles naissent des rapports de match : ' +
            'une fois les rapports déposés, l\'organisateur lance le calcul depuis l\'onglet Statistiques.');
        return;
    }

    // Cumul par sportif, avec le moteur partage
    const parSportif = {};
    stats.forEach(function(l) {
        if (!l.player_id) return;
        if (!parSportif[l.player_id]) parSportif[l.player_id] = [];
        parSportif[l.player_id].push(l);
    });

    const cumuls = Object.keys(parSportif).map(function(id) {
        const cumul = GTStats.agregerTournoi(parSportif[id], {
            tournament_id: currentTournamentId,
            player_id: id
        });
        cumul.cards = (cumul.yellow_cards || 0) + (cumul.red_cards || 0);
        return cumul;
    });

    const playerIds = cumuls.map(function(c) { return c.player_id; });
    const { data: profiles } = await supabaseClient
        .from(TBL_PROFILES).select('auth_uuid, full_name, avatar_url').in('auth_uuid', playerIds);
    const profileMap = {};
    (profiles || []).forEach(function(p) { profileMap[p.auth_uuid] = p; });

    renderPlayerRanking('scorersRanking', cumuls, profileMap, 'goals',   'But(s)',    'fa-futbol');
    renderPlayerRanking('assistsRanking', cumuls, profileMap, 'assists', 'Passe(s)',  'fa-hands-helping');
    renderPlayerRanking('cardsRanking',   cumuls, profileMap, 'cards',   'Carton(s)', 'fa-square');
    renderPlayerRanking('notesRanking',   cumuls, profileMap, 'average_rating', 'Note', 'fa-star-half-stroke');
}

function renderPlayerRanking(containerId, cumuls, profileMap, statKey, label, icon) {
    const container = document.getElementById(containerId);
    if (!container) return;

    const estUneNote = statKey === 'average_rating';

    // Une ligne a zero n'a rien a faire dans un classement de
    // buteurs. Une note, elle, se classe meme basse : c'est un
    // classement de performance, pas de compteur.
    let liste = cumuls.filter(function(c) {
        if (estUneNote) return c.average_rating != null;
        return Number(c[statKey]) > 0;
    });

    liste = GTStats.classer(liste, statKey);
    ...
    let html = '<table class="ranking-table player-ranking"><thead><tr>' +
               '<th>#</th><th>' + escapeHtml(mot('{Sportif}')) + '</th><th class="tabular">M</th><th>' + escapeHtml(label) + '</th>' +
               '</tr></thead><tbody>';

    liste.forEach(function(ligne) {
        const profile = profileMap[ligne.player_id] || {};
        const valeur = estUneNote
            ? '<span class="gt-note gt-note-sm ' + GTStats.classeNote(ligne.average_rating) + '">' +
              Number(ligne.average_rating).toFixed(2) + '</span>'
            : ligne[statKey];

        html += '<tr>' +
                '<td class="tabular">' + ligne.rang + '</td>' +
                '<td class="player-cell">' +
                (profile.avatar_url
                    ? '<img src="' + profile.avatar_url + '" alt="">'
                    : '<span class="avatar-initials-small">' + getInitials(profile.full_name || '?') + '</span>') +
                '<a href="player-stats.html?id=' + encodeURIComponent(ligne.player_id) + '">' +
                escapeHtml(profile.full_name || mot('{Sportif} inconnu')) + '</a>' +
                '</td>' +
                '<td class="tabular">' + (ligne.matches_played || 0) + '</td>' +
                '<td class="tabular points-col">' + valeur + '</td>' +
                '</tr>';
    });
...
function renderEmptyPlayerRankings(message) {
    ['scorersRanking', 'assistsRanking', 'cardsRanking', 'notesRanking'].forEach(function(id) {
        const el = document.getElementById(id);
        if (el) el.innerHTML = '<div class="empty-state"><i class="fas fa-chart-line"></i><p>' + escapeHtml(message) + '</p></div>';
    });
}
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    document.getElementById('tournamentSelect')?.addEventListener('change', async function() {
        ...
        // Le vocabulaire avant les donnees : sans cet await, le
        // tableau s'ecrirait avec le mot du tournoi precedent.
