...
async function loadRankings() {
    ...
    const { data: standings } = await supabaseClient
        ...
        .select('team_id, played, wins, draws, losses, goals_for, goals_against, points, supabaseAuthPrive_gt_teams!inner(name)')
...
function renderTeamsRanking(standings) {
    ...
    standings.forEach(function(s, index) {
        html += '<tr>' +
                '<td>' + (index + 1) + '</td>' +
                '<td>' + escapeHtml(s.supabaseAuthPrive_gt_teams.name) + '</td>' +
                '<td>' + s.played + '</td>' +
                '<td>' + s.wins + '</td>' +
                '<td>' + s.draws + '</td>' +
                '<td>' + s.losses + '</td>' +
