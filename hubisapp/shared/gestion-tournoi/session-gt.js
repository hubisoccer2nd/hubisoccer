...
async function loadTournamentContext(tournamentId) {
    ...
    const { data: t, error } = await supabaseClient
        ...
        .select('*, sport:gestionnairetournoi_sports(id, name), type:gestionnairetournoi_types(id, name, label)')
        .eq('id', tournamentId)
        .maybeSingle();

    if (error || !t) return { tournament: null, access: 'none' };

    // Organisateur ? On accepte les trois colonnes historiques
    // pour rester compatible avec les tournois déjà en base.
    const moi = currentUser ? currentUser.id : null;
    const estProprietaire = !!moi && (
        t.created_by   === moi ||
        t.organizer_id === moi ||
        t.user_id      === moi
    );
...
function isTournamentOwner(tournament) {
    ...
    return tournament.created_by === moi
        || tournament.organizer_id === moi
        || tournament.user_id === moi;
}
