/* ============================================================
   ...
   ------------------------------------------------------------
   Correction critique : la comparaison "Footballeurs" etait un
...
// ═══════════════════════════════════════════════════════════
// 12. CHARGEMENT DES ENTITÉS (ÉQUIPES OU FOOTBALLEURS)
// ═══════════════════════════════════════════════════════════
async function loadEntities() {
    ...
    if (currentCompareType === 'teams') {
        ...
        allPlayers = (profiles || []).map(function(p) { return { id: p.auth_uuid, name: p.full_name || 'Footballeur inconnu', avatar_url: p.avatar_url }; });
...
// ═══════════════════════════════════════════════════════════
// 15. CALCUL STATISTIQUES FOOTBALLEUR (réel — remplace le stub)
// ═══════════════════════════════════════════════════════════
async function computePlayerStats(playerId) {
    const stats = { matchs: 0, buts: 0, passes: 0, jaunes: 0, rouges: 0 };
    if (!tournamentMatchIds.length) return stats;
    ...
    (data || []).forEach(function(s) {
        stats.matchs++;
        stats.buts += s.goals || 0;
        stats.passes += s.assists || 0;
        stats.jaunes += s.yellow_cards || 0;
        stats.rouges += s.red_cards || 0;
    });
    return stats;
}
...
// ═══════════════════════════════════════════════════════════
// 17. RENDU HTML — FOOTBALLEURS
// ═══════════════════════════════════════════════════════════
function renderPlayerComparisonHTML(playerA, playerB, statsA, statsB) {
    const rows = [
        { label: 'Matchs joués', a: statsA.matchs, b: statsB.matchs },
        { label: 'Buts', a: statsA.buts, b: statsB.buts },
        { label: 'Passes décisives', a: statsA.passes, b: statsB.passes },
        { label: 'Cartons jaunes', a: statsA.jaunes, b: statsB.jaunes },
        { label: 'Cartons rouges', a: statsA.rouges, b: statsB.rouges }
    ];
