/* ============================================================
   ...
   Système Gestion Tournois — Détails d'un tournoi
   ------------------------------------------------------------
   Correction critique : chaque requete utilisait encore
   gestionnairetournoi_* (avant la migration) -> la requete
   principale echouait systematiquement, declenchant le retour
   automatique vers acceuil.html des l'ouverture de la page.
   Toutes les tables corrigees vers supabaseAuthPrive_gt_*.
   Section "Recompenses" retiree : jamais alimentee par aucun
   code, jamais branchee -- code mort, pas une fonctionnalite
   existante.
   ============================================================ */
'use strict';
...
// (identique a acceuil.js / create-tournament.js)
// ═══════════════════════════════════════════════════════════
...
// Statut temporel reel, calcule depuis les dates (comme acceuil.js)
...
async function loadTournamentDetails(tournamentId) {
    ...
    const { data, error } = await supabaseClient
        ...
        .select('id, name, description, start_date, end_date, location, registration_code, prize_pool, stream_url, requires_first_pas, has_agreed_to_rules, status, type_id, sport_id, ' + TBL_TYPES + '!inner(name, label), ' + TBL_SPORTS + '!inner(name)')
        .eq('id', tournamentId)
        .single();
    ...
    document.getElementById('tournamentDescription').textContent = data.description || 'Aucune description fournie.';
...
function renderHero(data) {
    ...
    if (state === 'ongoing') {
        pill.className = 'hero-state-pill ongoing';
        pill.innerHTML = '<span class="live-dot small"></span> En cours';
    } else if (state === 'upcoming') {
        pill.className = 'hero-state-pill upcoming';
        pill.innerHTML = '<i class="fas fa-clock"></i> À venir';
    } else if (state === 'past') {
        pill.className = 'hero-state-pill past';
        pill.innerHTML = '<i class="fas fa-flag-checkered"></i> Terminé';
    } else {
        pill.className = 'hero-state-pill';
        pill.innerHTML = '<i class="fas fa-question-circle"></i> —';
    }
}
...
async function loadMatches(tournamentId) {
    ...
    document.getElementById('statMatches').textContent = (!error && data)
    ...
    container.innerHTML = data.map(function(match) {
        ...
        const badge = isLive
            ? '<span class="match-live-badge"><span class="live-dot small"></span> Live</span>'
            : isDone
                ? '<span class="match-status-badge done">Terminé</span>'
                : '<span class="match-status-badge scheduled">' + date + '</span>';
...
async function loadStandings(tournamentId) {
    const { data, error } = await supabaseClient
        .from(TBL_STANDINGS)
        .select('id, played, wins, draws, losses, goals_for, goals_against, points, team:' + TBL_TEAMS + '!team_id(name)')
        .eq('tournament_id', tournamentId)
    ...
    if (error || !data || data.length === 0) {
        container.innerHTML = '<div class="empty-state"><i class="fas fa-chart-bar"></i><p>Aucun classement disponible pour le moment.</p></div>';
        return;
    }

    let html = '<table class="standings-table"><thead><tr>' +
               '<th class="col-rank">#</th><th class="col-team">Équipe</th>' +
               '<th>J</th><th>V</th><th>N</th><th>D</th><th>BP</th><th>BC</th><th class="col-pts">Pts</th>' +
               '</tr></thead><tbody>';

    data.forEach(function(row, index) {
        const zone = index === 0 ? 'zone-first' : index < 3 ? 'zone-podium' : '';
        html += '<tr class="' + zone + '">' +
                '<td class="col-rank tabular">' + (index + 1) + '</td>' +
                ...
                '<td class="col-pts tabular"><strong>' + (row.points || 0) + '</strong></td>' +
                '</tr>';
    });
    html += '</tbody></table>';
