...
async function loadTournamentDetails(tournamentId) {
    ...
    renderHero(data);
    afficherLaBandePublique(tournamentId, data);
...
// ═══════════════════════════════════════════════════════════
// ═══════════════════════════════════════════════════════════
// CHANTIER 17 — LE LIEN PUBLIC
// -----------------------------------------------------------
// Cette page-ci exige une connexion pour presque tout ce
// qu'elle propose. tournoi-public.html, non : c'est la version
// qu'on envoie aux parents, aux sponsors, aux spectateurs.
//
// La bande n'apparait que sur un tournoi REELLEMENT public.
// Donner un lien qui affichera « ce tournoi n'est pas encore
// public » serait pire que de ne rien donner.
//
// is_public peut ne pas exister encore (le chantier 17
// l'ajoute) : undefined vaut donc « public ».
// ═══════════════════════════════════════════════════════════
function lienPublicDuTournoi(tournamentId) {
    const base = window.location.href.split('?')[0].split('#')[0];
    return base.replace(/[^/]*$/, 'tournoi-public.html') + '?id=' + encodeURIComponent(tournamentId);
}

function afficherLaBandePublique(tournamentId, tournoi) {
    const bande = document.getElementById('bandeLienPublic');
    if (!bande) return;

    const estPublic = tournoi && tournoi.status === 'published' && tournoi.is_public !== false;
    if (!estPublic) { bande.style.display = 'none'; return; }

    const lien = lienPublicDuTournoi(tournamentId);
    bande.style.display = 'flex';

    const champ = document.getElementById('champLienPublic');
    if (champ) champ.value = lien;

    const ouvrir = document.getElementById('btnVuePublique');
    if (ouvrir) ouvrir.href = lien;

    const copier = document.getElementById('btnCopierLienPublic');
    if (copier && !copier.dataset.branche) {
        copier.dataset.branche = '1';
        copier.addEventListener('click', async function() {
            try {
                ...
                showToast('Lien public copié. Il s\'ouvre sans compte.', 'success');
            } catch (e) {
                // Le presse-papier est refuse hors HTTPS et sur
                // certains navigateurs mobiles. On ne dit pas
                // « copié » quand rien ne l'a ete.
    ...
    const wa = document.getElementById('btnWhatsAppPublic');
    if (wa && !wa.dataset.branche) {
        wa.dataset.branche = '1';
        wa.addEventListener('click', function() {
            // wa.me : aucune cle d'API, aucun compte professionnel.
            const texte = 'Suis ' + ((tournoi && tournoi.name) || 'ce tournoi') +
                          ' en direct sur HubISoccer : ' + lien;
...
async function loadBracket(tournamentId) {
    ...
    if (error) {
        // « Indisponible pour le moment » ne disait pas POURQUOI :
        // le lecteur attendait que ça revienne tout seul.
        signalerEchec('Le tableau final n\'a pas pu être chargé', error,
                      { zone: 'bracketZone', icone: 'fa-sitemap' });
        return;
    }
...
async function loadAwards(tournamentId) {
    ...
    // CHANTIER 15 — la récompense dit désormais QUI l'a gagnée.
    // Avant, la page annonçait « 1ère place : trophée + 500 000 F »
    // sans jamais nommer le vainqueur : le tournoi se jouait et le
    // palmarès n'existait nulle part.
    function gagnantHtml(a) {
        if (!a.winner_name) return '';
        return '<span class="award-winner"><i class="fas fa-crown"></i> ' +
    ...
    rankContainer.innerHTML = rankAwards.length ? rankAwards.map(function(a) {
        ...
        return '<div class="award-card' + (a.winner_name ? ' award-attribuee' : '') + '"><i class="fas ' + icon + '"></i>' +
               ...
               (a.amount ? '<span class="award-amount tabular">' + formatMoney(a.amount) + ' FCFA</span>' : '') +
               gagnantHtml(a) + '</div></div>';
    ...
    if (specialAwards.length) {
        ...
        specialContainer.innerHTML = specialAwards.map(function(a) {
            return '<div class="award-card special' + (a.winner_name ? ' award-attribuee' : '') + '"><i class="fas fa-star"></i>' +
