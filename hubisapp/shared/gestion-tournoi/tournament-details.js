...
async function loadTournamentDetails(tournamentId) {
    ...
    const { data, error } = await supabaseClient
        ...
        .select('*, ' + TBL_TYPES + '!inner(name, label), ' + TBL_SPORTS + '!inner(name)')
        .eq('id', tournamentId)
        .single();
...
async function loadStandings(tournamentId) {
    ...
    let html = '<table class="standings-table"><thead><tr><th class="col-rank">#</th><th class="col-team">Équipe</th><th>J</th><th>V</th><th>N</th><th>D</th><th>BP</th><th>BC</th><th class="col-pts">Pts</th></tr></thead><tbody>';
