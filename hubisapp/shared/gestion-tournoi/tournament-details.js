...
async function loadTournamentDetails(tournamentId) {
    ...
    const { data, error } = await supabaseClient
        ...
        .select('id, name, description, start_date, end_date, location, registration_code, prize_pool, stream_url, requires_first_pas, has_agreed_to_rules, type_id, sport_id, supabaseAuthPrive_gt_types!inner(name, label), supabaseAuthPrive_gt_sports!inner(name)')
        .eq('id', tournamentId)
        .single();
    ...
    document.getElementById('tournamentType').textContent = data.supabaseAuthPrive_gt_types ? data.supabaseAuthPrive_gt_types.label : 'Non spécifié';
    document.getElementById('tournamentSport').textContent = data.supabaseAuthPrive_gt_sports ? data.supabaseAuthPrive_gt_sports.name : 'Non spécifié';
...
async function loadMatches(tournamentId) {
    const { data, error } = await supabaseClient
        ...
        .select('id, round, team_a_id, team_b_id, score_a, score_b, match_date, status, team_a:supabaseAuthPrive_gt_teams!team_a_id(name), team_b:supabaseAuthPrive_gt_teams!team_b_id(name)')
        .eq('tournament_id', tournamentId)
...
async function loadStandings(tournamentId) {
    const { data, error } = await supabaseClient
        .from('supabaseAuthPrive_gt_standings')
        .select('id, played, wins, draws, losses, goals_for, goals_against, points, team:supabaseAuthPrive_gt_teams!team_id(name)')
        .eq('tournament_id', tournamentId)
