/* ============================================================
   ...
   suit la discipline du tournoi (chantier 08), routage
   dynamique profil/parametres
...
// Chantier 08 — la discipline. Cette page n'est pas rattachee a
// UN tournoi : elle liste les equipes que l'utilisateur a creees,
// qui peuvent appartenir a plusieurs tournois, donc a plusieurs
// disciplines. On ne nomme le pratiquant que si toutes ces
// equipes relevent de la MEME discipline. Des qu'il y en a deux,
// on s'en tient au terme neutre « sportif » : mieux vaut un mot
// juste pour tous qu'un mot faux pour la moitie.
let nomSportTournoi = '';
...
async function resoudreDiscipline(equipes) {
    nomSportTournoi = '';
    try {
        const idsTournois = [...new Set((equipes || [])
            .map(e => e.tournament_id).filter(Boolean))];
        if (!idsTournois.length) { appliquerLexique(); return; }

        const { data: tournois } = await supabaseClient
            .from(TBL_TOURNAMENTS)
            .select('id, sport_id')
            .in('id', idsTournois);

        const idsSports = [...new Set((tournois || [])
            .map(t => t.sport_id).filter(Boolean))];
        if (idsSports.length !== 1) { appliquerLexique(); return; }
        ...
        // Une discipline non resolue n'empeche pas un transfert :
        // on garde le terme neutre et la page continue.
        signalerEchec('Le sport du tournoi n\'a pas pu être lu — les mots affichés ' +
                      'resteront génériques', e, { ton: 'warning' });
    }
    appliquerLexique();
}
...
async function loadUserTeamsAndPlayers() {
    ...
    if (error) { signalerEchec('Tes équipes n\'ont pas pu être chargées', error); return; }

    userTeams = teams || [];
    // Chantier 08 — le vocabulaire d'abord : tout ce qui s'ecrit
    // ensuite (liste deroulante, messages) doit deja porter le
    // bon mot.
    await resoudreDiscipline(userTeams);
    ...
    if (playersError) { signalerEchec('L\'effectif de tes équipes n\'a pas pu être chargé', playersError); return; }
    ...
    players.forEach(p => {
        ...
        playersMap[p.user_id] = {
            name: profileMap[p.user_id] || mot('{Sportif} inconnu'),
...
function populatePlayerSelect() {
    ...
    select.innerHTML = '<option value="">' + escapeHtml(mot('Sélectionnez votre {sportif}')) + '</option>';
...
async function loadAllTeams() {
    ...
    if (error) { signalerEchec('La liste des équipes du tournoi n\'a pas pu être chargée', error); return; }
...
async function submitTransferRequest(event) {
    ...
    if (!playerId || !targetTeamId) {
        showToast(mot('Veuillez sélectionner un {sportif} et une {collectif} de destination'), 'warning');
        return;
    }
    ...
    if (player.fromTeamId == targetTeamId) {
        showToast(mot('Ce {sportif} est déjà dans cette {collectif}'), 'warning');
        return;
    }
    ...
    if (error) {
        // Le motif, et pas seulement « erreur » : sans lui,
        // personne ne sait s'il faut recommencer ou appeler.
        signalerEchec('La demande de transfert n\'a pas été envoyée', error);
        return;
    }
    ...
    data.forEach(req => {
        ...
        card.innerHTML =
            '<div class="request-header">' +
                '<span class="player-name">' + escapeHtml(profileMap[req.player_id] || mot('{Sportif} inconnu')) + '</span>' +
...
async function handleTransferDecision(requestId, newStatus) {
    ...
    if (newStatus === 'approved') {
        ...
        if (request.to_team_id) {
            ...
            if (insertError) {
                hideLoader();
                showToast(mot('Erreur lors du transfert du {sportif}'), 'error');
                return;
            }
        }
    }
