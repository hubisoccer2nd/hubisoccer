/* ============================================================
   HubISoccer -- staff-certif.js
   Espace Staff Medical - Diplomes & Certificats
   Adapte de foot-certif.js : porte d'acces role_code='FOOT'
   retiree (aucun autre fichier de cet espace ne l'utilise), types de
   document adaptes au contexte staff medical (formation continue/
   autre au lieu de scolaire/sportif/autre), chemin de stockage
   propre a ce role. Reste du mecanisme (table partagee
   certifications filtree par user_hubisoccer_id, bucket
   documents, glisser-deposer) conserve a l'identique.
   ============================================================ */
...
async function loadProfile() {
    ...
    userProfile = data;
    document.getElementById('userName').textContent = userProfile.full_name || userProfile.display_name || 'Staff Médical';
    updateAvatarUI();
    return userProfile;
}
...
function renderCertificates() {
    ...
    container.innerHTML = certificates.map(cert => {
        ...
        if (cert.type === 'formation') icon = 'fa-chalkboard-teacher';
        else if (cert.type === 'specialisation') icon = 'fa-award';
        else if (cert.type === 'diplome') icon = 'fa-graduation-cap';
        return `
...
function initUploadForm() {
    ...
    document.getElementById('certForm').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const filePath = `certifications/staff_medical/${fileName}`;
