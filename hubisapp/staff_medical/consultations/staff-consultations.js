/* ============================================================
   HubISoccer -- staff-consultations.js
   Consultations - Espace Staff Medical
   ------------------------------------------------------------
   Journal LOGISTIQUE uniquement : date/type/athlete/statut
   aptitude/tarif -- jamais de diagnostic. Le champ tarif
   alimentera Mes Honoraires (calcul automatique depuis cette
   table). Selecteur d'athlete depuis le roster deja accepte.
   ============================================================ */
'use strict';
...
const APTITUDE_ICONS = {
    'Apte': 'fa-check-circle',
    'Apte sous réserve': 'fa-exclamation-triangle',
    'Non apte': 'fa-times-circle',
    "En attente d'examen": 'fa-hourglass-half'
};
...
async function loadEntries() {
    ...
    if (error) {
        ...
        showToast('Table absente. Exécutez le script SQL <b>staff-consultations-table.sql</b> dans Supabase.', 'warning');
        allEntries = [];
        return;
    }
    allEntries = data || [];
...
function updateStats() {
    ...
    const entreesMois = allEntries.filter(function(e) {
        if (!e.date_consultation) return false;
        const d = new Date(e.date_consultation);
        return d.getMonth() === m && d.getFullYear() === y;
    });
    document.getElementById('statMois').textContent = entreesMois.length;

    const honorairesMois = entreesMois.reduce(function(s, e) { return s + (parseFloat(e.tarif) || 0); }, 0);
    document.getElementById('statHonorairesMois').textContent = formatMoney(honorairesMois);
...
function renderAll() {
    ...
    const filtered = allEntries.filter(function(e) {
        ...
        const matchFilter = !filter || (e.statut_aptitude === filter);
        return matchSearch && matchFilter;
    });
    const grid = document.getElementById('entriesGrid');
    grid.innerHTML = '';
    if (!filtered.length) {
        grid.innerHTML = '<div class="empty-state"><i class="fas fa-notes-medical"></i><p>Aucun résultat.</p></div>';
        return;
    }
    filtered.forEach(function(item) {
        ...
        meta += '<span><i class="fas ' + (APTITUDE_ICONS[item.statut_aptitude] || 'fa-question-circle') + '"></i>' + item.statut_aptitude + '</span>';
        if (item.tarif) meta += '<span><i class="fas fa-coins"></i>' + formatMoney(item.tarif) + '</span>';
        card.innerHTML =
            '<div class="entry-card-header">' +
            '<span class="entry-card-title">' + item.athlete_nom + '</span>' +
            '<span class="entry-badge">' + item.type_consultation + '</span>' +
            '</div>' +
...
function openAdd() {
    editingId = null;
    document.getElementById('modalTitle').innerHTML = '<i class="fas fa-plus"></i> Nouvelle consultation';
...
function openEdit(id) {
    ...
    document.getElementById('f_type_consultation').value = item.type_consultation || '';
    document.getElementById('f_date_consultation').value = item.date_consultation || '';
    document.getElementById('f_statut_aptitude').value = item.statut_aptitude || "En attente d'examen";
    document.getElementById('f_tarif').value = item.tarif ?? '';
...
async function saveEntry() {
    ...
    if (!document.getElementById('f_type_consultation').value) { showToast('Sélectionnez un type de consultation.', 'warning'); return; }
    if (!document.getElementById('f_date_consultation').value) { showToast('La date est obligatoire.', 'warning'); return; }

    const data = {
        athlete_id            : athleteId,
        athlete_nom              : athleteOpt ? athleteOpt.textContent : '',
        type_consultation           : document.getElementById('f_type_consultation').value,
        date_consultation              : document.getElementById('f_date_consultation').value,
        statut_aptitude                   : document.getElementById('f_statut_aptitude').value,
        tarif                                : document.getElementById('f_tarif').value || null,
    ...
    showToast(editingId ? 'Entrée modifiée !' : 'Consultation enregistrée !', 'success');
...
function initFilters() {
    ...
    ['Apte', 'Apte sous réserve', 'Non apte', "En attente d'examen"].forEach(function(statut) {
        const opt = document.createElement('option');
        opt.value = statut;
