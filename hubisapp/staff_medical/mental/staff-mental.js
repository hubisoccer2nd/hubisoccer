/* ============================================================
   HubISoccer -- staff-mental.js
   Preparation Mentale - Espace Staff Medical
   ------------------------------------------------------------
   Journal de seances de techniques de performance mentale --
   jamais un suivi psychologique clinique. Selecteur d'athlete
   depuis le roster deja accepte, meme pattern que Traumatologie/
   Soins/Wellness.
   ============================================================ */
'use strict';
...
let rosterList     = [];
let editingId          = null;
const TABLE      = 'supabaseAuthPrive_staff_medical_mental';
...
async function loadEntries() {
    ...
    if (error) {
        ...
        showToast('Table absente. Exécutez le script SQL <b>staff-mental-table.sql</b> dans Supabase.', 'warning');
        allEntries = [];
        return;
    }
    allEntries = data || [];
...
function updateStats() {
    ...
    const semaine = allEntries.filter(function(e) {
        return e.date_seance && new Date(e.date_seance) >= debutSemaine;
    }).length;
    document.getElementById('statSemaine').textContent = semaine;

    document.getElementById('statTechniques').textContent = new Set(allEntries.map(function(e) { return e.type_technique; })).size;
...
function renderAll() {
    ...
    const filtered = allEntries.filter(function(e) {
        ...
        const matchFilter = !filter || (e.type_technique === filter);
        return matchSearch && matchFilter;
    });
    const grid = document.getElementById('entriesGrid');
    grid.innerHTML = '';
    if (!filtered.length) {
        grid.innerHTML = '<div class="empty-state"><i class="fas fa-brain"></i><p>Aucun résultat.</p></div>';
        return;
    }
    filtered.forEach(function(item) {
        const card = document.createElement('div');
        card.className = 'entry-card';
        const dateStr = item.date_seance ? new Date(item.date_seance).toLocaleDateString('fr-FR') : '';
        let meta = '';
        if (dateStr) meta += '<span><i class="fas fa-calendar-alt"></i>' + dateStr + '</span>';
        if (item.contexte) meta += '<span><i class="fas fa-tag"></i>' + item.contexte + '</span>';
        if (item.duree_minutes) meta += '<span><i class="fas fa-clock"></i>' + item.duree_minutes + ' min</span>';
        if (item.progres_observe) meta += '<span><i class="fas fa-chart-line"></i>Progrès : ' + item.progres_observe + '</span>';
        card.innerHTML =
            '<div class="entry-card-header">' +
            '<span class="entry-card-title">' + item.athlete_nom + '</span>' +
            '<span class="entry-badge">' + item.type_technique + '</span>' +
            '</div>' +
            '<div class="entry-meta">' + meta + '</div>' +
            (item.objectif ? '<div class="entry-desc" style="font-size:.8rem;color:var(--gray-dark);margin-top:6px;"><i class="fas fa-bullseye"></i> ' + item.objectif + '</div>' : '') +
...
function openEdit(id) {
    ...
    document.getElementById('f_type_technique').value = item.type_technique || '';
    document.getElementById('f_date_seance').value = item.date_seance || '';
    document.getElementById('f_duree_minutes').value = item.duree_minutes || '';
    document.getElementById('f_contexte').value = item.contexte || 'Entraînement';
    document.getElementById('f_objectif').value = item.objectif || '';
    document.getElementById('f_progres_observe').value = item.progres_observe || '';
...
async function saveEntry() {
    ...
    if (!document.getElementById('f_type_technique').value) { showToast('Sélectionnez une technique.', 'warning'); return; }
    if (!document.getElementById('f_date_seance').value) { showToast('La date est obligatoire.', 'warning'); return; }

    const data = {
        athlete_id            : athleteId,
        athlete_nom              : athleteOpt ? athleteOpt.textContent : '',
        type_technique              : document.getElementById('f_type_technique').value,
        date_seance                    : document.getElementById('f_date_seance').value,
        duree_minutes                     : document.getElementById('f_duree_minutes').value || null,
        contexte                             : document.getElementById('f_contexte').value,
        objectif                                : document.getElementById('f_objectif').value || null,
        progres_observe                            : document.getElementById('f_progres_observe').value || null,
...
function initFilters() {
    ...
    ['Cohérence cardiaque', 'Sophrologie', 'Imagerie mentale', 'Visualisation', 'Gestion du stress', 'Concentration / Focus', 'Autre'].forEach(function(type) {
        const opt = document.createElement('option');
        opt.value = type;
