/* ============================================================
   HubISoccer -- staff-profile.js
   Page Mon Profil - Espace Staff Medical
...
async function saveProfile(e) {
    ...
    Object.assign(staffProfile, updates);
}
...
function updateDeletionUI() {
    ...
    if (staffProfile.deletion_requested && !staffProfile.deletion_revoked) {
        if (delBtn) { delBtn.style.display = 'none'; }
        if (revokeBtn) { revokeBtn.style.display = 'inline-flex'; }
        if (status && staffProfile.deletion_requested_at) {
            const reqDate = new Date(staffProfile.deletion_requested_at);
...
async function confirmerSuppression() {
    ...
    if (error) { showToast('Erreur : ' + error.message, 'error'); return; }
    staffProfile.deletion_requested = true;
    staffProfile.deletion_requested_at = new Date().toISOString();
    staffProfile.deletion_revoked = false;
...
async function revokeDeletion() {
    ...
    if (error) { showToast('Erreur : ' + error.message, 'error'); return; }
    staffProfile.deletion_requested = false;
    staffProfile.deletion_revoked = true;
    updateDeletionUI();
