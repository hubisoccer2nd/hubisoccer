/* ============================================================
   HubISoccer — staff-revenus.js
   Dashboard principal du HubIS Wallet – Staff Médical
   ============================================================ */
...
function updateNavbarAvatar() {
    ...
    if (userName) userName.textContent = userProfile.full_name || userProfile.display_name || 'Staff Médical';
    const avatarUrl = userProfile.avatar_url;
    if (avatarUrl && avatarUrl !== '') {
        if (userAvatar) { userAvatar.src = avatarUrl; userAvatar.style.display = 'block'; }
        if (userInitials) userInitials.style.display = 'none';
    } else {
        const initials = getInitials(userProfile.full_name || userProfile.display_name || 'C');
...
async function loadWallet() {
    ...
    if (!data || error) {
        ...
        setTimeout(function() { window.location.href = 'staff-revenus-setup.html'; }, 1500);
        return null;
    }
