/* ============================================================
   HubISoccer -- staff-supp.js
   Page Support - Espace Staff Medical
   ------------------------------------------------------------
   Convention tables : supabaseAuthPrive_[espace]_[page]
   - supabaseAuthPrive_profiles        -> partagee (lecture)
   - supabaseAuthPrive_staff_medical_support -> table de CETTE page
     (SQL : staff-supp-table.sql, sans RLS)
   ------------------------------------------------------------
   La FAQ est statique (aucune table) -- elle synthetise les 9
   pages precedentes de l'espace staff medical.
   ============================================================ */
'use strict';
...
const SUPPORT_TABLE  = 'supabaseAuthPrive_staff_medical_support';

/* ---------- 3. FAQ (STATIQUE -- synthetise l'espace staff medical) ---------- */
const FAQ_ITEMS = [
    {
        q: 'Comment suivre un nouvel athlete ?',
        a: 'Reperez-le dans "Scouting", consultez sa fiche puis cliquez sur "Suivre cet athlete". Une demande lui est envoyee -- il doit l\'accepter pour rejoindre votre roster dans "Mes Athletes Suivis".'
    },
    {
        q: 'Comment obtenir mon statut verifie ?',
        a: 'Dans "Verification du Statut" : renseignez votre specialite et votre numero d\'Ordre, televersez les 6 documents demandes (dont le casier judiciaire, obligatoire pour ce role), signez, puis soumettez. L\'equipe HubISoccer examine le dossier sous 0 a 100h.'
    },
    {
        q: 'Qu\'est-ce que le Pass Medical (vert/orange/rouge) ?',
        a: 'C\'est un badge resume de disponibilite pour chaque athlete de votre roster, dans "Mes Athletes Suivis". Vert = operationnel, orange = prudence, rouge = indisponible. La categorie de raison reste generale -- jamais un diagnostic.'
    },
    {
        q: 'Comment enregistrer une blessure et suivre le retour au jeu ?',
        a: '"Traumatologie & Suivi RTP" consigne type, zone et gravite en categories generales, plus l\'etape actuelle du protocole de retour -- de "Repos complet" a "Retabli".'
    },
    {
        q: 'Comment fonctionnent mes honoraires ?',
        a: 'Enregistrez le tarif de chaque consultation dans "Consultations" -- "Mes Honoraires" calcule tout automatiquement (total, ce mois, payes, non payes) sans rien re-saisir.'
    },
    {
        q: 'Comment activer mon Wallet HubIS ?',
        a: '"Mes Revenus" vous guide dans un assistant d\'activation (creation du wallet, code PIN) puis affiche votre carte, votre solde et vos transactions.'
    },
    {
        q: 'Ou suivre le bien-etre et le sommeil de mes athletes ?',
        a: '"Wellness Quotidien" pour l\'humeur, le stress, l\'energie et la recuperation percue -- "Neuro-Recuperation & Sommeil" specifiquement pour les heures de sommeil et la fatigue nerveuse.'
    },
    {
        q: 'Comment televerser mes diplomes et mon CV ?',
        a: 'Dans "Diplomes & Certificats", ajoutez formation continue, certifications ou diplomes -- le statut passe a "Valide" une fois verifie. "Mon CV Pro" reprend votre parcours complet, avec export PDF.'
    }
];
...
/* ---------- 9. CHARGEMENT PROFIL STAFF MEDICAL ---------- */
...
function updateNavbarAvatar() {
    ...
    if (url && url !== '') {
        ...
        const init = getInitials(staffProfile?.full_name || 'P');
...
async function loadTickets() {
    ...
    if (error) {
        ...
        showToast('Table du support absente. Executez le script SQL <b>staff-supp-table.sql</b> dans Supabase.', 'warning');
        allTickets = [];
        return;
    }
    allTickets = data || [];
