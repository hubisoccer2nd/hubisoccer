/* ============================================================
   HubISoccer -- staff-wellness.js
   Wellness Quotidien - Espace Staff Medical
   ------------------------------------------------------------
   Auto-evaluation quotidienne (humeur/stress/energie/recuperation
   sur 1-10) -- un ressenti, jamais un diagnostic. Selecteur
   d'athlete depuis le roster deja accepte, meme pattern que
   Traumatologie et Soins.
   ============================================================ */
'use strict';
...
let rosterList     = [];
let editingId          = null;
const TABLE      = 'supabaseAuthPrive_staff_medical_wellness';
...
async function loadEntries() {
    ...
    const { data, error } = await supabaseClient
        ...
        .select('*')
        .eq('staff_medical_id', staffProfile.hubisoccer_id)
        .order('date_wellness', { ascending: false });
    hideLoader();
    if (error) {
        console.warn('Table ' + TABLE + ' absente :', error.message);
        showToast('Table absente. Exécutez le script SQL <b>staff-wellness-table.sql</b> dans Supabase.', 'warning');
        allEntries = [];
        return;
    }
    allEntries = data || [];
...
function updateStats() {
    ...
    const semaine = allEntries.filter(function(e) {
        return e.date_wellness && new Date(e.date_wellness) >= debutSemaine;
    }).length;
    document.getElementById('statSemaine').textContent = semaine;

    const humeurs = allEntries.map(function(e) { return e.score_humeur; }).filter(function(v) { return v !== null && v !== undefined; });
    const moyenne = humeurs.length ? (humeurs.reduce(function(s, v) { return s + v; }, 0) / humeurs.length) : 0;
    document.getElementById('statHumeurMoy').textContent = humeurs.length ? moyenne.toFixed(1) : '—';
...
function renderAll() {
    ...
    grid.innerHTML = '';
    if (!filtered.length) {
        grid.innerHTML = '<div class="empty-state"><i class="fas fa-heart"></i><p>Aucun résultat.</p></div>';
        return;
    }
    filtered.forEach(function(item) {
        const card = document.createElement('div');
        card.className = 'entry-card';
        const dateStr = item.date_wellness ? new Date(item.date_wellness).toLocaleDateString('fr-FR') : '';
        const moyGlobale = ((item.score_humeur + item.niveau_energie + item.qualite_recuperation + (11 - item.niveau_stress)) / 4).toFixed(1);
        let meta = '';
        if (dateStr) meta += '<span><i class="fas fa-calendar-alt"></i>' + dateStr + '</span>';
        meta += '<span><i class="fas fa-smile"></i>Humeur : ' + item.score_humeur + '/10</span>';
        meta += '<span><i class="fas fa-bolt"></i>Énergie : ' + item.niveau_energie + '/10</span>';
        meta += '<span><i class="fas fa-brain"></i>Stress : ' + item.niveau_stress + '/10</span>';
        meta += '<span><i class="fas fa-battery-full"></i>Récupération : ' + item.qualite_recuperation + '/10</span>';
        card.innerHTML =
            '<div class="entry-card-header">' +
            '<span class="entry-card-title">' + item.athlete_nom + '</span>' +
            '<span class="entry-badge">Score global : ' + moyGlobale + '/10</span>' +
            '</div>' +
...
function openAdd() {
    editingId = null;
    document.getElementById('modalTitle').innerHTML = '<i class="fas fa-plus"></i> Nouvelle entrée wellness';
...
function openEdit(id) {
    ...
    document.getElementById('f_date_wellness').value = item.date_wellness || '';
    document.getElementById('f_score_humeur').value = item.score_humeur ?? '';
    document.getElementById('f_niveau_stress').value = item.niveau_stress ?? '';
    document.getElementById('f_niveau_energie').value = item.niveau_energie ?? '';
    document.getElementById('f_qualite_recuperation').value = item.qualite_recuperation ?? '';
...
async function saveEntry() {
    ...
    const champs = ['f_date_wellness', 'f_score_humeur', 'f_niveau_stress', 'f_niveau_energie', 'f_qualite_recuperation'];
    ...
    const data = {
        ...
        date_wellness              : document.getElementById('f_date_wellness').value,
        score_humeur                  : parseInt(document.getElementById('f_score_humeur').value, 10),
        niveau_stress                    : parseInt(document.getElementById('f_niveau_stress').value, 10),
        niveau_energie                      : parseInt(document.getElementById('f_niveau_energie').value, 10),
        qualite_recuperation                   : parseInt(document.getElementById('f_qualite_recuperation').value, 10),
